...
test('adds and safely deletes an unused account', async ({ page }) => {
  ...
  await accountRow(page, '999901').getByRole('button', { name: 'Delete Account' }).click();
  await page.getByRole('button', { name: 'Save', exact: true }).click();
...
test('validates duplicate codes and blocks deletion of referenced accounts', async ({ page }) => {
  ...
  await accountRow(page, '1507').getByRole('button', { name: 'Delete Account' }).click();
  await page.getByRole('button', { name: 'Save', exact: true }).click();
  await expect(page.locator('#coaMessage')).toContainText(/cannot be deleted.*Deactivate it instead/i);
