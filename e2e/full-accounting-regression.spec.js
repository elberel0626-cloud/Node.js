...
test('SO invoice preparation and posting count invoiced quantity exactly once',async({page})=>{
  ...
  await api(page,'POST','/api/sales-orders/action',{id:so.orderNumber||so.id||soNumber,action:'Confirm'});
  await api(page,'POST','/api/sales-orders/create-shipment',{salesOrderId:so.orderNumber||so.id||soNumber,shipDate:DATE,confirm:true});
  const shipments=await api(page,'GET','/api/sales-orders/shipments');
  const shipment=shipments.filter(row=>row.salesOrderId===(so.id||so.orderNumber||soNumber)||row.salesOrderNumber===soNumber).at(-1);
  ...
  const prepared=await api(page,'POST','/api/sales-orders/prepare-invoice',{salesOrderId:so.orderNumber||so.id||soNumber,shipmentId:shipment.id,invoiceNumber,date:DATE,postDate:DATE});
  const orderAfterPrepare=await api(page,'GET',`/api/sales-orders/${encodeURIComponent(so.orderNumber||so.id||soNumber)}`);
  ...
  const orderAfterPost=await api(page,'GET',`/api/sales-orders/${encodeURIComponent(so.orderNumber||so.id||soNumber)}`);
