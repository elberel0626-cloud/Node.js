...
test('normal AP payments do not require approval while prepayments still do', () => {
...
if(['Payment','Prepayment'].includes(doc.type)){syncApPaymentReview(doc);const payStatus=doc.paymentApprovalStatus||'Not Required';if(payStatus==='Pending Payment Approval')throw apPostingBusinessError('Payment batch requires payment approval before posting.');}`;
  ...
  assert.match(patched, /if\(doc\.type==='Prepayment'\)/);
  assert.match(patched, /Vendor prepayment requires payment approval before posting/);
  assert.doesNotMatch(patched, /Payment batch requires payment approval before posting/);
});
