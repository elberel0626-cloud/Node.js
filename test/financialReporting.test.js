...
test('mapped financial reports never change sections just because an account is contra or negative',()=>{
 ...
 assert.equal(sales.subgroups.find(group=>group.name==='Equipment').details[0].accountCode,'4008');
 assert.equal(sales.subgroups.find(group=>group.name==='Sales Discount').details[0].accountCode,'4025');
 const cogs=pl.sections[1].groups.find(group=>group.name==='COGS');
 const sga=pl.sections[1].groups.find(group=>group.name==='SG&A');
 const nonEbitda=pl.sections[1].groups.find(group=>group.name==='NON EBITDA');
 assert.equal(cogs.total,500);
 assert.equal(cogs.subgroups.find(group=>group.name==='Material').details[0].accountCode,'5110');
 assert.equal(sga.total,200);
 assert.equal(sga.subgroups.find(group=>group.name==='Marketing').details[0].accountCode,'6010');
 assert.equal(nonEbitda.total,50);
 assert.equal(nonEbitda.subgroups.find(group=>group.name==='Amortization').details[0].accountCode,'6031');
