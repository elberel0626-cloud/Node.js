...
const responseHeaders=(res)=>Object.fromEntries(res.headers.entries());
...
export class AzureDocumentAIProvider extends DocumentAIProvider {
  ...
  async analyzeInvoice(document,mimeType='application/pdf'){
    ...
    const requestHeaders={'Ocp-Apim-Subscription-Key':this.key,'Content-Type':'application/pdf'};
    const sanitizedRequestHeaders=Object.fromEntries(Object.entries(requestHeaders).filter(([key])=>key.toLowerCase()!=='ocp-apim-subscription-key'));
    console.log('[invoice-recognition-trace] azure-fetch-code', "fetch(url,{method:'POST',headers:{'Ocp-Apim-Subscription-Key':'<redacted>','Content-Type':'application/pdf'},body:document})");
    const operation=await this.retryFetch(url,{method:'POST',headers:requestHeaders,body:document});
    ...
    logAzureExchange({ requestUrl:url, method:'POST', apiVersion:this.apiVersion, model:this.model, requestHeaders:sanitizedRequestHeaders, httpStatus:operation.status, providerRequestId, responseHeaders:responseHeaders(operation), responseBody:operationBody??operationText });
    ...
    for(let i=0;i<40;i++){
      ...
      console.log('[invoice-recognition-trace] azure-poll-fetch-code', "fetch(operationLocation,{headers:{'Ocp-Apim-Subscription-Key':'<redacted>'}})");
      ...
      logAzureExchange({ requestUrl:operationLocation, method:'GET', apiVersion:this.apiVersion, model:this.model, requestHeaders:{}, httpStatus:poll.status, providerRequestId:pollRequestId, responseHeaders:responseHeaders(poll), responseBody:json??pollText });
