...
export function applyManufacturingAgent3MrpExceptionsRuntimePatch(source){
  ...
  function mrpFirmSupplies(){const rows=[];for(const line of purchaseOrderLines){const po=purchaseOrders.find(row=>row.id===line.poId);if(!po||!activePoStatus(po.status))continue;const quantity=Math.max(0,Number(line.qtyOpen??(Number(line.qtyOrdered||0)-Number(line.qtyReceived||0)-Number(line.qtyCancelled||0))));if(quantity<=0)continue;rows.push({supplyType:'Purchase Order',reference:po.poNumber||po.id,lineReference:line.id,itemId:line.inventoryId||line.itemId,quantity:qty(quantity),date:line.requestedDate||po.promisedDate||po.requestedDate||po.orderDate||datePlusDays(nowDate(),settings.planningHorizonDays),status:po.status});}for(const order of orders.filter(row=>activeMfgStatus(row.status))){const quantity=Math.max(0,Number(order.quantity||0)-Number(order.qtyCompleted||0)-Number(order.qtyScrapped||0));if(quantity<=0)continue;rows.push({supplyType:'Production Order',reference:order.id,lineReference:'',itemId:order.itemId,quantity:qty(quantity),date:order.dueDate||datePlusDays(nowDate(),settings.planningHorizonDays),status:order.status});}return rows;}
