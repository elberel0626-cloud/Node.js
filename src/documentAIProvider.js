...
class LocalPdfDocumentAIProvider {
  async analyzeInvoice(document, mimeType = 'application/pdf') {
    ...
    try {
      ...
      if (!text.trim() && hasCommand('pdftoppm') && hasCommand('tesseract')) {
        execFileSync('pdftoppm', ['-png', '-r', '200', file, path.join(dir, 'page')], { stdio: 'ignore' });
        text = pages.map((_, i) => { try { return execFileSync('tesseract', [path.join(dir, `page-${i + 1}.png`), 'stdout'], { encoding: 'utf8', maxBuffer: 10 * 1024 * 1024 }); } catch { return ''; } }).join('\n');
      }
