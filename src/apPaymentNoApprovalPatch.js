...
export function applyApPaymentNoApprovalPatch(source) {
  ...
  source = replaceOnce(
    ...
    "if(['Payment','Prepayment'].includes(doc.type)){syncApPaymentReview(doc);if(doc.type==='Prepayment'){const payStatus=doc.paymentApprovalStatus||'Pending Payment Approval';if(payStatus==='Pending Payment Approval')throw apPostingBusinessError('Vendor prepayment requires payment approval before posting.');}}",
    'posting approval gate applies only to vendor prepayments'
  );
