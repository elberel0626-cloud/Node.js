...
// AP invoice recognition (OCR/AI abstraction with deterministic local fallback for demo/runtime use)
const invoiceRecognitionConfig={OCRProvider:process.env.OCR_PROVIDER||'Tesseract',autoApprovalConfidence:0.95,vendorFuzzyThreshold:0.90,lineMatchThreshold:0.85,twoWayTolerancePct:5,taxTolerance:0.05,companyCurrency:'USD',supportedCurrencies:['USD','CAD','EUR','GBP','MXN']};
const invoiceRecognitionDocuments=[];
const invoiceRecognitionFields=[];
const invoiceRecognitionLines=[];
const invoiceRecognitionExceptions=[];
const invoiceRecognitionAudit=[];
const invoiceRecognitionJobs=[];
const fieldVal=(value,confidence=0.96,page=1,bbox=null)=>({value:value??'',confidence,page,boundingBox:bbox||{x:0,y:0,width:0,height:0}});
const norm=(s)=>String(s||'').toLowerCase().replace(/[^a-z0-9]+/g,' ').trim();
function levenshtein(a='',b=''){a=norm(a);b=norm(b);const m=Array.from({length:a.length+1},(_,i)=>[i]);for(let j=1;j<=b.length;j++)m[0][j]=j;for(let i=1;i<=a.length;i++)for(let j=1;j<=b.length;j++)m[i][j]=Math.min(m[i-1][j]+1,m[i][j-1]+1,m[i-1][j-1]+(a[i-1]===b[j-1]?0:1));return m[a.length]?.[b.length]??0;}
function similarity(a,b){a=norm(a);b=norm(b);if(!a&&!b)return 1;if(!a||!b)return 0;return 1-levenshtein(a,b)/Math.max(a.length,b.length,1);}
function auditRecognition(documentId,action,user='system',changes={},reason=''){invoiceRecognitionAudit.push({id:`IRA-${invoiceRecognitionAudit.length+1001}`,documentId,action,user,timestamp:new Date().toISOString(),fieldChanged:changes.field||'',oldValue:changes.oldValue??'',newValue:changes.newValue??'',reason});}
function extractFromText(text=''){
 const line=(rx)=>String(text).match(rx)?.[1]?.trim()||''; const amount=(rx)=>Number((line(rx)||'0').replace(/[^0-9.-]/g,''));
 const vendorName=line(/(?:vendor|from|supplier)\s*:?\s*([^\n]+)/i)||String(text).split(/\r?\n/).find(Boolean)||'Unknown Vendor';
 const poNumber=line(/(?:po|purchase order)\s*(?:number|#|no)?\s*:?\s*(PO[-\w]+)/i);
 const total=amount(/(?:total|amount due)\s*:?\s*\$?([\d,]+(?:\.\d{2})?)/i);
 const subtotal=amount(/subtotal\s*:?\s*\$?([\d,]+(?:\.\d{2})?)/i)||total;
 const tax=amount(/(?:tax|vat|gst)\s*:?\s*\$?([\d,]+(?:\.\d{2})?)/i);
 const currency=(String(text).match(/\b(USD|CAD|EUR|GBP|MXN)\b/i)?.[1]||'USD').toUpperCase();
 const rawLines=[]; String(text).split(/\r?\n/).forEach((l,i)=>{const m=l.match(/^\s*(?:line\s*)?(\d+)?\s*([^|,]+?)\s+(\d+(?:\.\d+)?)\s+(EA|HR|PC|PCS)?\s*\$?([\d,.]+)\s+\$?([\d,.]+)\s*$/i); if(m) rawLines.push({lineNumber:rawLines.length+1,description:m[2].trim(),quantity:Number(m[3]),unitOfMeasure:m[4]||'EA',unitPrice:Number(m[5].replace(/,/g,'')),discount:0,tax:0,lineAmount:Number(m[6].replace(/,/g,'')),vendorItemNumber:'',poLineReference:'',confidence:0.91,page:1});});
 const po=purchaseOrders.find(p=>p.poNumber===poNumber); if(!rawLines.length&&po){purchaseOrderLines.filter(l=>l.poId===po.id).forEach((l,i)=>rawLines.push({lineNumber:i+1,description:l.description,quantity:l.qtyOrdered,unitOfMeasure:l.uom,unitPrice:l.unitCost,discount:l.discountAmount||0,tax:l.tax||0,lineAmount:l.extendedCost,vendorItemNumber:l.vendorItemId||'',poLineReference:l.lineId,confidence:0.93,page:1}));}
 return {provider:invoiceRecognitionConfig.OCRProvider,rawText:text,fields:{vendorName:fieldVal(vendorName,0.94),vendorTaxId:fieldVal(line(/(?:tax id|vat id|ein)\s*:?\s*([\w-]+)/i),0.88),invoiceNumber:fieldVal(line(/(?:invoice|inv)\s*(?:number|#|no)?\s*:?\s*([\w-]+)/i)||`OCR-${Date.now()}`,0.97),invoiceDate:fieldVal(line(/invoice date\s*:?\s*(\d{4}-\d{2}-\d{2}|\d{1,2}\/\d{1,2}\/\d{2,4})/i)||new Date().toISOString().slice(0,10),0.96),dueDate:fieldVal(line(/due date\s*:?\s*(\d{4}-\d{2}-\d{2}|\d{1,2}\/\d{1,2}\/\d{2,4})/i),0.9),poNumber:fieldVal(poNumber,poNumber?0.96:0.5),currency:fieldVal(currency,0.98),subtotal:fieldVal(subtotal,0.94),taxAmount:fieldVal(tax,0.93),freightAmount:fieldVal(amount(/freight\s*:?\s*\$?([\d,]+(?:\.\d{2})?)/i),0.9),discountAmount:fieldVal(amount(/discount\s*:?\s*\$?([\d,]+(?:\.\d{2})?)/i),0.9),totalAmount:fieldVal(total||subtotal+tax,0.95)},lines:rawLines};
}
function matchVendor(fields){const name=fields.vendorName.value,taxId=fields.vendorTaxId.value;let exact=vendors.find(v=>norm(v.name)===norm(name));if(exact)return {vendor:exact,confidence:1,method:'Exact Match'};let tax=taxId&&vendors.find(v=>String(v.taxId||'')===taxId);if(tax)return {vendor:tax,confidence:1,method:'Tax ID Match'};const ranked=vendors.map(v=>({vendor:v,confidence:similarity(v.name,name),method:'Fuzzy Match'})).filter(x=>x.confidence>=invoiceRecognitionConfig.vendorFuzzyThreshold).sort((a,b)=>b.confidence-a.confidence);return ranked.length===1?ranked[0]:{vendor:ranked[0]?.vendor||null,confidence:ranked[0]?.confidence||0,method:ranked.length>1?'Multiple Matches':'No Match',multiple:ranked.length>1};}
function lineMatch(invLine,po){const lines=purchaseOrderLines.filter(l=>l.poId===po?.id);let ranked=lines.map(l=>({poLine:l,score:Math.max(String(invLine.vendorItemNumber||'')&&invLine.vendorItemNumber===l.vendorItemId?1:0,String(invLine.poLineReference||'')&&invLine.poLineReference===l.lineId?0.98:0,similarity(invLine.description,l.description))})).sort((a,b)=>b.score-a.score)[0];return ranked&&ranked.score>=invoiceRecognitionConfig.lineMatchThreshold?ranked:null;}
function validateRecognition(doc,result){const exceptions=[];const vm=matchVendor(result.fields);const po=purchaseOrders.find(p=>p.poNumber===result.fields.poNumber.value)||purchaseOrders.find(p=>vm.vendor&&p.vendorId===vm.vendor.id&&Math.abs(Number(p.poTotal||0)-Number(result.fields.totalAmount.value||0))/Math.max(Number(p.poTotal||1),1)<=0.05);const existing=apDocuments.find(d=>d.vendorId===vm.vendor?.id&&d.invoiceNumber===result.fields.invoiceNumber.value&&d.id!==doc.billId);if(existing)exceptions.push({type:'Duplicate',severity:'Block',message:`Exact duplicate invoice ${existing.id}`}); if(!vm.vendor||vm.multiple)exceptions.push({type:'Vendor Match',severity:'Review',message:vm.method}); if(!po)exceptions.push({type:'PO Match',severity:'Review',message:'Purchase order could not be matched'}); const lineTax=result.lines.reduce((s,l)=>s+Number(l.tax||0),0); if(Math.abs(lineTax-Number(result.fields.taxAmount.value||0))>invoiceRecognitionConfig.taxTolerance)exceptions.push({type:'Tax Validation',severity:'Review',message:'Line tax total does not equal invoice tax total'}); if(!invoiceRecognitionConfig.supportedCurrencies.includes(result.fields.currency.value))exceptions.push({type:'Currency',severity:'Review',message:'Unsupported currency'}); const totalCalc=Number(result.fields.subtotal.value||0)+Number(result.fields.taxAmount.value||0)+Number(result.fields.freightAmount.value||0)-Number(result.fields.discountAmount.value||0); if(Math.abs(totalCalc-Number(result.fields.totalAmount.value||0))>0.05)exceptions.push({type:'Total Validation',severity:'Review',message:'Header totals do not foot'}); if(po){const diff=Math.abs(Number(po.poTotal||0)-Number(result.fields.totalAmount.value||0))/Math.max(Number(po.poTotal||1),1)*100;if(diff>invoiceRecognitionConfig.twoWayTolerancePct)exceptions.push({type:'2-Way Match',severity:'Review',message:`Invoice total differs from PO by ${diff.toFixed(2)}%`}); result.lines.forEach(l=>{const m=lineMatch(l,po); if(m){l.matchedPOLineId=m.poLine.id;l.matchScore=m.score;} else exceptions.push({type:'Line Match',severity:'Review',message:`No PO line match for ${l.description}`});});}
 const fieldConfs=Object.values(result.fields).map(f=>Number(f.confidence||0)); const lineConfs=result.lines.map(l=>Number(l.confidence||0)); const confidence=[...fieldConfs,...lineConfs].reduce((s,c)=>s+c,0)/Math.max(fieldConfs.length+lineConfs.length,1); return {vendorMatch:vm,po,exceptions,confidence};}
function createBillFromRecognition(doc,result,validation){const vendor=validation.vendorMatch.vendor;if(!vendor)throw new Error('Vendor match required before bill creation'); const id=`BILL-${String(apDocuments.length+1001).padStart(4,'0')}`; const f=result.fields; const bill={id,type:'Bill',vendorId:vendor.id,vendorName:vendor.name,date:f.invoiceDate.value,postDate:f.invoiceDate.value,postPeriod:periodFromDate(f.invoiceDate.value),dueDate:f.dueDate.value||f.invoiceDate.value,terms:vendor.terms||'NET30',status:'Saved',posted:false,amount:Number(f.totalAmount.value||0),balance:Number(f.totalAmount.value||0),vendorRef:f.invoiceNumber.value,invoiceNumber:f.invoiceNumber.value,currency:f.currency.value,exchangeRate:f.currency.value===invoiceRecognitionConfig.companyCurrency?1:({EUR:1.09,GBP:1.27,CAD:0.73,MXN:0.059}[f.currency.value]||1),invoicePdfAttached:true,attachmentName:doc.originalFileName,source:'Invoice Recognition',recognitionDocumentId:doc.id,sourcePoId:validation.po?.id||'',lines:result.lines.map(l=>({inventoryId:purchaseOrderLines.find(pl=>pl.id===l.matchedPOLineId)?.inventoryId||'',description:l.description,qty:l.quantity,unitCost:l.unitPrice,lineTotal:l.lineAmount,poLineId:l.matchedPOLineId,poId:validation.po?.id||'',account:purchaseOrderLines.find(pl=>pl.id===l.matchedPOLineId)?.expenseAccount||'5110'}))}; apDocuments.push(bill); doc.billId=id; auditRecognition(doc.id,'Automatic bill creation','system',{field:'billId',newValue:id},'High confidence recognition'); return bill;}
function processRecognitionDocument(doc,text){const result=extractFromText(text||doc.rawText||''); const validation=validateRecognition(doc,result); doc.vendorId=validation.vendorMatch.vendor?.id||''; doc.invoiceNumber=result.fields.invoiceNumber.value; doc.confidenceScore=validation.confidence; doc.processedDate=new Date().toISOString(); doc.extraction=result; doc.status=(validation.confidence>=invoiceRecognitionConfig.autoApprovalConfidence&&validation.exceptions.length===0)?'Approved':'Needs Review'; Object.entries(result.fields).forEach(([k,f])=>invoiceRecognitionFields.push({id:`IRF-${invoiceRecognitionFields.length+1001}`,documentId:doc.id,fieldName:k,value:f.value,confidence:f.confidence,page:f.page,boundingBox:f.boundingBox})); result.lines.forEach((l,i)=>invoiceRecognitionLines.push({id:`IRL-${invoiceRecognitionLines.length+1001}`,documentId:doc.id,lineNumber:i+1,...l,amount:l.lineAmount,matchedPOLineId:l.matchedPOLineId||''})); validation.exceptions.forEach(e=>invoiceRecognitionExceptions.push({id:`IRE-${invoiceRecognitionExceptions.length+1001}`,documentId:doc.id,status:e.severity==='Block'?'New':'Needs Review',...e,createdDate:new Date().toISOString()})); if(doc.status==='Approved')createBillFromRecognition(doc,result,validation); auditRecognition(doc.id,'OCR and validation','system',{field:'status',newValue:doc.status},`${result.provider} provider`); return serializeRecognition(doc);}
function serializeRecognition(doc){return {...doc,fields:invoiceRecognitionFields.filter(f=>f.documentId===doc.id),lines:invoiceRecognitionLines.filter(l=>l.documentId===doc.id),exceptions:invoiceRecognitionExceptions.filter(e=>e.documentId===doc.id),audit:invoiceRecognitionAudit.filter(a=>a.documentId===doc.id),bill:apDocuments.find(b=>b.id===doc.billId)||null};}
...
const server=http.createServer(async(req,res)=>{const {pathname,query}=parse(req.url,true); const method=req.method||'GET'; try{
 ...
 // AP Invoice Recognition APIs
 if(method==='GET'&&pathname==='/api/ap/invoices/config') return json(res,200,invoiceRecognitionConfig);
 if(method==='PUT'&&pathname==='/api/ap/invoices/config'){ Object.assign(invoiceRecognitionConfig,await body(req)); return json(res,200,invoiceRecognitionConfig); }
 if(method==='POST'&&pathname==='/api/ap/invoices/upload'){ const b=await body(req); const id=`IRD-${String(invoiceRecognitionDocuments.length+1001).padStart(4,'0')}`; const guid=crypto.randomUUID?crypto.randomUUID():`${Date.now()}-${Math.random()}`; const doc={id,documentGuid:guid,originalFileName:b.fileName||b.file||'invoice.pdf',originalPdfPath:`encrypted://ap-invoices/${guid}.pdf`,status:'processing',confidenceScore:0,createdDate:new Date().toISOString(),processedDate:'',vendorId:'',invoiceNumber:'',rawText:b.text||b.ocrText||'',ocrProvider:b.ocrProvider||invoiceRecognitionConfig.OCRProvider,security:{tls:'TLS 1.3 required at ingress',encryptedStorage:true,roles:['AP Clerk','AP Manager','Controller','Administrator']}}; invoiceRecognitionDocuments.push(doc); invoiceRecognitionJobs.push({id:`IRQ-${invoiceRecognitionJobs.length+1001}`,documentId:id,status:'processing',createdDate:doc.createdDate}); auditRecognition(id,'Upload',b.userId||'ap.clerk',{field:'OriginalPdfPath',newValue:doc.originalPdfPath},'PDF uploaded'); processRecognitionDocument(doc,doc.rawText); invoiceRecognitionJobs.find(j=>j.documentId===id).status='completed'; return json(res,201,{documentId:doc.documentGuid,id:doc.id,status:doc.status,confidenceScore:doc.confidenceScore,billId:doc.billId||'',reviewRequired:doc.status==='Needs Review'}); }
 if(method==='GET'&&pathname==='/api/ap/invoices/recognition') return json(res,200,invoiceRecognitionDocuments.map(serializeRecognition));
 if(method==='GET'&&pathname.startsWith('/api/ap/invoices/recognition/')){ const id=pathname.split('/').pop(); const doc=invoiceRecognitionDocuments.find(d=>d.id===id||d.documentGuid===id); return doc?json(res,200,serializeRecognition(doc)):json(res,404,{error:'Recognition document not found'}); }
 if(method==='POST'&&pathname.startsWith('/api/ap/invoices/recognition/')&&pathname.endsWith('/rerun')){ const id=pathname.split('/')[5]; const doc=invoiceRecognitionDocuments.find(d=>d.id===id||d.documentGuid===id); if(!doc) return json(res,404,{error:'Recognition document not found'}); doc.status='processing'; auditRecognition(doc.id,'Re-run OCR','ap.clerk'); return json(res,200,processRecognitionDocument(doc,doc.rawText)); }
 if(method==='PUT'&&pathname.startsWith('/api/ap/invoices/recognition/')){ const id=pathname.split('/').pop(); const doc=invoiceRecognitionDocuments.find(d=>d.id===id||d.documentGuid===id); if(!doc) return json(res,404,{error:'Recognition document not found'}); const b=await body(req); for(const [k,v] of Object.entries(b.fields||{})){ const f=invoiceRecognitionFields.find(x=>x.documentId===doc.id&&x.fieldName===k); if(f){auditRecognition(doc.id,'Edit','ap.clerk',{field:k,oldValue:f.value,newValue:v},b.reason||'Human review correction'); f.value=v; f.confidence=1;} } (b.lines||[]).forEach(l=>{ const row=invoiceRecognitionLines.find(x=>x.id===l.id||x.lineNumber===l.lineNumber&&x.documentId===doc.id); if(row){ Object.entries(l).forEach(([k,v])=>{ if(row[k]!==v){auditRecognition(doc.id,'Edit','ap.clerk',{field:`line.${k}`,oldValue:row[k],newValue:v},b.reason||'Human review correction'); row[k]=v; }}); row.confidence=1; }}); doc.status='Corrected'; return json(res,200,serializeRecognition(doc)); }
 if(method==='POST'&&pathname.startsWith('/api/ap/invoices/recognition/')&&pathname.endsWith('/approve')){ const id=pathname.split('/')[5]; const doc=invoiceRecognitionDocuments.find(d=>d.id===id||d.documentGuid===id); if(!doc) return json(res,404,{error:'Recognition document not found'}); const fields=Object.fromEntries(invoiceRecognitionFields.filter(f=>f.documentId===doc.id).map(f=>[f.fieldName,fieldVal(f.value,1,f.page,f.boundingBox)])); const result={fields,lines:invoiceRecognitionLines.filter(l=>l.documentId===doc.id),provider:doc.ocrProvider}; const validation=validateRecognition(doc,result); if(validation.exceptions.some(e=>e.severity==='Block')) return json(res,409,{error:'Blocking exceptions must be resolved before approval',exceptions:validation.exceptions}); const bill=doc.billId?apDocuments.find(b=>b.id===doc.billId):createBillFromRecognition(doc,result,validation); doc.status='Posted'; auditRecognition(doc.id,'Approval','ap.manager',{field:'status',newValue:'Posted'},'Human reviewed and approved'); return json(res,200,{document:serializeRecognition(doc),bill}); }
 if(method==='POST'&&pathname.startsWith('/api/ap/invoices/recognition/')&&pathname.endsWith('/reject')){ const id=pathname.split('/')[5]; const doc=invoiceRecognitionDocuments.find(d=>d.id===id||d.documentGuid===id); if(!doc) return json(res,404,{error:'Recognition document not found'}); doc.status='Rejected'; auditRecognition(doc.id,'Reject','ap.manager',{field:'status',newValue:'Rejected'},(await body(req)).reason||'Rejected in review'); return json(res,200,serializeRecognition(doc)); }
 if(method==='GET'&&pathname==='/api/ap/invoices/exceptions') return json(res,200,invoiceRecognitionExceptions);
 if(method==='GET'&&pathname==='/api/ap/invoices/audit') return json(res,200,invoiceRecognitionAudit.filter(a=>!query.documentId||a.documentId===query.documentId));
