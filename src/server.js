...
function receiptUnbilledLines(receiptId){
  return purchaseReceiptLines.filter(l=>!receiptId||l.receiptId===receiptId).map(l=>{ const poLine=purchaseOrderLines.find(x=>x.id===l.poLineId); const qtyBilled=Math.min(Number(l.receiptQty||0),Number(poLine?.qtyBilled||0)); return {...l,qtyReceived:Number(l.receiptQty||0),qtyBilled,qtyRemaining:Math.max(0,Number(l.receiptQty||0)-qtyBilled),poNumber:purchaseOrders.find(p=>p.id===l.poId)?.poNumber||l.poId}; }).filter(l=>l.qtyRemaining>0);
}
function billableLinesForPo(po){
  return purchaseOrderLines.filter(l=>l.poId===po.id).map(l=>{ const qtyRemaining=Math.max(0,Number(l.qtyReceived||0)-Number(l.qtyBilled||0)); const receipt=purchaseReceipts.find(r=>r.poId===po.id&&purchaseReceiptLines.some(rl=>rl.receiptId===r.id&&rl.poLineId===l.id&&Number(rl.receiptQty||0)>Number(rl.qtyBilled||0))); return {poLineId:l.id,lineNumber:l.lineNumber,poNumber:po.poNumber,receiptNumber:receipt?.receiptNumber||'',inventoryId:l.inventoryId,description:l.description,qtyReceived:Number(l.qtyReceived||0),qtyBilled:Number(l.qtyBilled||0),qtyRemaining,qtyAvailableToBill:qtyRemaining,uom:l.uom,warehouse:l.warehouse,location:l.location,unitCost:l.unitCost,rniAccount:l.apAccrualAccount,expenseAccount:l.apAccrualAccount,status:l.status}; }).filter(l=>l.qtyRemaining>0);
}
...
function apBillExceptions(doc){
  ...
  for(const line of lines){
    ...
    if(po&&!receipt) reasons.push('Receipt missing');
    if(poLine){
      const poQty=Number(poLine.qtyReceived||poLine.qtyOrdered||0); const poUnit=Number(poLine.unitCost||0);
...
function processApBillPoMatches(doc){
  ...
  for(const line of doc.lines||[]){
    ...
    const qty=Number(line.qty||line.quantity||0); const available=Math.max(0,Number(poLine.qtyReceived||0)-Number(poLine.qtyBilled||0));
    if(qty>available) throw new Error('Cannot bill more than received quantity.');
...
const server=http.createServer(async(req,res)=>{const {pathname,query}=parse(req.url,true); const method=req.method||'GET'; try{
 ...
 if(method==='GET'&&pathname==='/api/purchase-orders/receipts') return json(res,200,purchaseReceipts.map(r=>({...r,lines:purchaseReceiptLines.filter(l=>l.receiptId===r.id)})));
