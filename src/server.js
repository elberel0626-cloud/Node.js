...
const server=http.createServer(async(req,res)=>{const {pathname,query}=parse(req.url,true); const method=req.method||'GET'; try{
 ...
 if(method==='PUT'&&pathname.startsWith('/api/ar/customers/')){ const id=pathname.split('/').pop(); const c=customers.find(x=>x.id===id); if(!c) return json(res,404,{error:'Customer not found'}); const update=await body(req); if(update.creditLimit!==undefined){const limit=Number(update.creditLimit); if(!Number.isFinite(limit)||limit<0)return json(res,400,{error:'Credit Limit must be zero or greater'}); update.creditLimit=limit;} if(update.creditHold!==undefined){update.creditHold=!!update.creditHold; if(c.status!=='Inactive')update.status=update.creditHold?'On Hold':'Active';} Object.assign(c,update); return json(res,200,enrichCustomer(c)); }
