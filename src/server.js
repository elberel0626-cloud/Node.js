...
function incomingStatusLabel(status){ return String(status||'UPLOADED').replaceAll('_',' '); }
function normalizeMoney(v){ return Math.round(Number(v||0)*100)/100; }
function nextIncomingDocumentId(){ return `INDOC-${String(incomingDocuments.length+1001).padStart(4,'0')}`; }
function recordIncomingAudit(documentId,userId,fieldChanged,oldValue,newValue){ incomingDocumentAudit.push({id:`IDAUD-${String(incomingDocumentAudit.length+1001).padStart(4,'0')}`,entity_type:'IncomingDocument',entity_id:documentId,documentId,user_id:userId||'ap.clerk',field_changed:fieldChanged,old_value:oldValue,new_value:newValue,timestamp:new Date().toISOString()}); }
function defaultExtractedInvoice(doc){
  const vendor=vendors.find(v=>String(doc.file_name||'').toLowerCase().includes(String(v.name||'').toLowerCase().split(/\s+/)[0]))||vendors[0]||{};
  const po=purchaseOrders.find(p=>p.vendorId===vendor.id)||purchaseOrders[0]||{};
  const poLines=purchaseOrderLines.filter(l=>l.poId===po.id).slice(0,2);
  const lineItems=poLines.length?poLines.map(l=>({description:l.description||l.inventoryId,quantity:Number(l.qtyOpen||l.qtyOrdered||1),unit_price:Number(l.unitCost||0),amount:normalizeMoney(Number(l.qtyOpen||l.qtyOrdered||1)*Number(l.unitCost||0)),po_number:po.poNumber||po.id,gl_suggested:l.expenseAccount||l.inventoryAccount||POSTING_ACCOUNTS.apTrade,confidence:0.9,bounding_box:{page:1,x:120,y:360,width:420,height:24}})):[{description:'Invoice services',quantity:1,unit_price:1250,amount:1250,po_number:'',gl_suggested:'5109',confidence:0.82,bounding_box:{page:1,x:120,y:360,width:420,height:24}}];
  const subtotal=normalizeMoney(lineItems.reduce((t,l)=>t+Number(l.amount||0),0)); const tax=normalizeMoney(subtotal*0.08); const total=normalizeMoney(subtotal+tax);
  return {vendor:{id:vendor.id||'',name:vendor.name||'',address:vendor.address||'',confidence:vendor.id?0.93:0.45,bounding_box:{page:1,x:72,y:96,width:260,height:32}},invoice:{invoice_number:`AI-${new Date().toISOString().slice(0,10).replaceAll('-','')}-${String(incomingDocuments.length+1).padStart(3,'0')}`,invoice_date:new Date().toISOString().slice(0,10),due_date:new Date(Date.now()+30*86400000).toISOString().slice(0,10),currency:vendor.currency||'USD',confidence:0.91,bounding_box:{page:1,x:390,y:96,width:160,height:80}},totals:{subtotal,tax,total,confidence:0.94,bounding_box:{page:1,x:410,y:650,width:150,height:70}},line_items:lineItems};
}
function validateIncomingDocument(doc){
  const extracted=doc.extracted_json||{}; const issues=[]; const vendor=vendors.find(v=>v.id===extracted.vendor?.id)||vendors.find(v=>String(v.name).toLowerCase()===String(extracted.vendor?.name||'').toLowerCase());
  if(!vendor) issues.push({code:'VENDOR_NOT_FOUND',severity:'error',message:'Vendor was not matched. Select or create a vendor.'});
  if(Number(extracted.vendor?.confidence||0)<0.85) issues.push({code:'LOW_VENDOR_CONFIDENCE',severity:'warning',message:'Vendor confidence is below 85%.'});
  const invoiceNumber=String(extracted.invoice?.invoice_number||'').trim(); if(!invoiceNumber) issues.push({code:'MISSING_INVOICE_NUMBER',severity:'error',message:'Invoice number is required.'});
  const subtotal=normalizeMoney((extracted.line_items||[]).reduce((t,l)=>t+Number(l.amount||0),0)); const tax=normalizeMoney(extracted.totals?.tax||0); const expected=normalizeMoney(subtotal+tax); const total=normalizeMoney(extracted.totals?.total||0);
  if(Math.abs(expected-total)>0.02) issues.push({code:'TOTAL_MISMATCH',severity:'error',message:`Total ${total.toFixed(2)} does not equal line subtotal plus tax ${expected.toFixed(2)}.`});
  if(vendor&&invoiceNumber&&apDocuments.some(d=>d.vendorId===vendor.id&&String(d.invoiceNumber||d.vendorRef||'').toLowerCase()===invoiceNumber.toLowerCase()&&d.status!=='Voided')) issues.push({code:'DUPLICATE_INVOICE',severity:'error',message:'Duplicate vendor invoice detected.'});
  (extracted.line_items||[]).forEach((line,i)=>{ if(line.po_number&&!purchaseOrders.find(p=>p.poNumber===line.po_number||p.id===line.po_number)) issues.push({code:'PO_NOT_FOUND',severity:'warning',line:i+1,message:`PO ${line.po_number} was not found.`}); });
  doc.validation_issues=issues; doc.confidence_score=normalizeMoney(Math.min(Number(extracted.vendor?.confidence||0.5),Number(extracted.invoice?.confidence||0.5),Number(extracted.totals?.confidence||0.5))*100); doc.status=issues.some(i=>i.severity==='error')?'NEEDS_REVIEW':'RECOGNIZED'; return issues;
}
function matchIncomingPoLines(doc){ const extracted=doc.extracted_json||{}; const matches=(extracted.line_items||[]).map((line,i)=>{ const po=purchaseOrders.find(p=>p.poNumber===line.po_number||p.id===line.po_number)||purchaseOrders.find(p=>p.vendorId===(extracted.vendor?.id||'')); const candidates=po?purchaseOrderLines.filter(l=>l.poId===po.id):[]; const best=candidates.find(l=>String(l.inventoryId||'').toLowerCase()===String(line.item||'').toLowerCase())||candidates.find(l=>String(l.description||'').toLowerCase()===String(line.description||'').toLowerCase())||candidates.slice().sort((a,b)=>Math.abs(Number(line.amount||0)-Number(a.extendedCost||0))-Math.abs(Number(line.amount||0)-Number(b.extendedCost||0)))[0]; const diff=best?Math.abs(Number(line.amount||0)-Number(best.extendedCost||0)):0; const tol=best?Math.max(1,Number(best.extendedCost||0)*0.02):0; const status=best?(diff<=tol?'Matched':'Partially matched'):'Not matched'; return {line:indexSafe(i),status,poNumber:po?.poNumber||line.po_number||'',poLineId:best?.id||'',description:best?.description||'',amountVariance:normalizeMoney(diff),tolerance:normalizeMoney(tol)}; }); doc.po_matches=matches; return matches; }
function indexSafe(i){ return Number(i)+1; }
async function storeIncomingDocumentFile({fileName='',contentBase64='',vendorId=''}){ const today=new Date().toISOString().slice(0,10); const cleanVendor=vendorId||'unmatched'; const id=nextIncomingDocumentId(); const ext=path.extname(fileName)||'.pdf'; const rel=path.join('document_storage',cleanVendor,today,`${id}${ext}`); const abs=path.resolve(rel); await mkdir(path.dirname(abs),{recursive:true}); const data=String(contentBase64||'').includes(',')?String(contentBase64).split(',').pop():String(contentBase64||''); await writeFile(abs,Buffer.from(data||'Uploaded AP invoice placeholder','base64')); return {id,fileUrl:`/${rel.replaceAll(path.sep,'/')}`,storagePath:rel}; }
...
const server=http.createServer(async(req,res)=>{const {pathname,query}=parse(req.url,true); const method=req.method||'GET'; try{
 ...
 if(method==='GET'&&pathname==='/api/ap/incoming-documents') return json(res,200,incomingDocuments.map(d=>({...d,status_label:incomingStatusLabel(d.status),audit:incomingDocumentAudit.filter(a=>a.documentId===d.id)})));
 if(method==='GET'&&pathname.startsWith('/api/ap/incoming-documents/')){ const id=pathname.split('/').pop(); const d=incomingDocuments.find(x=>x.id===id); return d?json(res,200,{...d,status_label:incomingStatusLabel(d.status),audit:incomingDocumentAudit.filter(a=>a.documentId===d.id)}):json(res,404,{error:'Incoming document not found'}); }
 if(method==='POST'&&pathname==='/api/ap/incoming-documents'){ const b=await body(req); const stored=await storeIncomingDocumentFile({fileName:b.fileName||b.name||'invoice.pdf',contentBase64:b.contentBase64||b.file||'',vendorId:b.vendorId||''}); const doc={id:stored.id,file_url:stored.fileUrl,file_name:b.fileName||b.name||'invoice.pdf',storage_path:stored.storagePath,status:'UPLOADED',created_at:new Date().toISOString(),uploaded_by:b.uploadedBy||'ap.clerk',extracted_json:null,confidence_score:0,validation_issues:[],po_matches:[],source_bounding_boxes:{}}; incomingDocuments.push(doc); recordIncomingAudit(doc.id,doc.uploaded_by,'status','',doc.status); return json(res,201,doc); }
 if(method==='POST'&&pathname.startsWith('/api/ap/incoming-documents/')&&pathname.endsWith('/recognize')){ const id=pathname.split('/')[4]; const doc=incomingDocuments.find(x=>x.id===id); if(!doc) return json(res,404,{error:'Incoming document not found'}); doc.status='PROCESSING'; doc.ocr_output={engine:'Simulated hybrid OCR',text_blocks:[{text:`Invoice candidate ${doc.file_name}`,confidence:0.91,page:1}],tables:[],coordinates:[]}; doc.extracted_json=defaultExtractedInvoice(doc); validateIncomingDocument(doc); recordIncomingAudit(doc.id,'ap.clerk','recognition','PROCESSING',doc.status); return json(res,200,doc); }
 if(method==='PUT'&&pathname.startsWith('/api/ap/incoming-documents/')){ const id=pathname.split('/').pop(); const doc=incomingDocuments.find(x=>x.id===id); if(!doc) return json(res,404,{error:'Incoming document not found'}); const b=await body(req); const before=JSON.stringify(doc.extracted_json||{}); if(b.extracted_json) doc.extracted_json=b.extracted_json; validateIncomingDocument(doc); recordIncomingAudit(doc.id,b.userId||'ap.clerk','extracted_json',before,JSON.stringify(doc.extracted_json||{})); return json(res,200,doc); }
 if(method==='POST'&&pathname.startsWith('/api/ap/incoming-documents/')&&pathname.endsWith('/match-po')){ const id=pathname.split('/')[4]; const doc=incomingDocuments.find(x=>x.id===id); if(!doc) return json(res,404,{error:'Incoming document not found'}); const matches=matchIncomingPoLines(doc); recordIncomingAudit(doc.id,'ap.clerk','po_matches','',JSON.stringify(matches)); return json(res,200,{matches,document:doc}); }
 if(method==='POST'&&pathname.startsWith('/api/ap/incoming-documents/')&&pathname.endsWith('/reject')){ const id=pathname.split('/')[4]; const doc=incomingDocuments.find(x=>x.id===id); if(!doc) return json(res,404,{error:'Incoming document not found'}); const old=doc.status; doc.status='REJECTED'; recordIncomingAudit(doc.id,'ap.clerk','status',old,doc.status); return json(res,200,doc); }
 if(method==='POST'&&pathname.startsWith('/api/ap/incoming-documents/')&&pathname.endsWith('/create-bill')){ const id=pathname.split('/')[4]; const doc=incomingDocuments.find(x=>x.id===id); if(!doc) return json(res,404,{error:'Incoming document not found'}); if(!doc.extracted_json) return json(res,400,{error:'Recognize the document before creating an AP bill.'}); validateIncomingDocument(doc); if(doc.validation_issues.some(i=>i.severity==='error')) return json(res,400,{error:'Resolve validation errors before creating AP Bill.',issues:doc.validation_issues}); const ex=doc.extracted_json; const vendor=vendors.find(v=>v.id===ex.vendor?.id)||vendors.find(v=>v.name===ex.vendor?.name); if(!vendor) return json(res,400,{error:'Vendor required'}); const amount=normalizeMoney(ex.totals?.total||0); const billId=`BILL-${String(apDocuments.length+1001).padStart(4,'0')}`; const bill={id:billId,type:'Bill',vendorId:vendor.id,vendorName:vendor.name,date:ex.invoice?.invoice_date||new Date().toISOString().slice(0,10),postDate:ex.invoice?.invoice_date||new Date().toISOString().slice(0,10),postPeriod:periodFromDate(ex.invoice?.invoice_date||new Date().toISOString().slice(0,10)),dueDate:ex.invoice?.due_date||ex.invoice?.invoice_date,terms:vendor.terms||'NET30',vendorRef:ex.invoice?.invoice_number||'',invoiceNumber:ex.invoice?.invoice_number||'',amount,balance:amount,status:amount>=10000?'Pending Approval':'Saved',posted:false,source:'Incoming Document AI',source_document_id:doc.id,sourceDocumentId:doc.id,invoicePdfAttached:true,attachmentName:doc.file_name,attachments:[{name:doc.file_name,url:doc.file_url,type:'Source Invoice'}],branch:'MAIN',department:'Finance',createdBy:'ap.clerk',currency:ex.invoice?.currency||vendor.currency||'USD',description:`AI recognized bill from ${doc.file_name}`,lines:(ex.line_items||[]).map((l,i)=>({lineNumber:i+1,description:l.description,qty:Number(l.quantity||0),unitCost:Number(l.unit_price||0),lineTotal:Number(l.amount||0),expenseAccount:l.gl_suggested||'5109',gl_account:l.gl_suggested||'5109',poReference:l.po_number||'',poId:l.po_number||'',sourceBoundingBox:l.bounding_box||null,confidence:l.confidence||0}))}; bill.approvalRequired=apApprovalRequired(bill); apDocuments.push(bill); const old=doc.status; doc.status='CONVERTED_TO_AP_BILL'; doc.ap_bill_id=bill.id; recordIncomingAudit(doc.id,'ap.clerk','status',old,doc.status); return json(res,201,{document:doc,bill:serializeApDoc(bill)}); }
