...
async function processIncomingInvoice(documentId, { retry = false } = {}){
  ...
  if(activeRecognitionDocuments.has(documentId)||['PROCESSING','Processing','QUEUED','Queued'].includes(r.processingStatus||r.status)){ const err=new Error('Invoice recognition is already in progress.'); err.code='RECOGNITION_ALREADY_PROCESSING'; err.statusCode=409; throw err; }
...
const server=http.createServer(async(req,res)=>{const {pathname,query}=parse(req.url,true); const method=req.method||'GET'; try{
 ...
 if(method==='POST'&&pathname.startsWith('/api/ap/incoming-documents/')&&pathname.endsWith('/process')){ const id=pathname.split('/').at(-2); try{return json(res,200,await processIncomingInvoice(id));}catch(e){return json(res,e.statusCode||400,{code:e.code||'RECOGNITION_FAILED',message:e.message,error:e.message});} }
 ...
 if(method==='POST'&&pathname.startsWith('/api/ap/incoming-documents/')&&pathname.endsWith('/retry-recognition')){ const id=pathname.split('/').at(-2); try{return json(res,200,await processIncomingInvoice(id,{retry:true}));}catch(e){return json(res,e.statusCode||400,{code:e.code||'RECOGNITION_FAILED',message:e.message,error:e.message});} }
