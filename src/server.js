...
function createReceiptFromPo(poId, opts={}){
  ...
  const receiptId=opts.receiptNumber||nextPoId('RCPT-PO',purchaseReceipts);
  const receipt={id:receiptId,receiptNumber:receiptId,poId:po.id,poNumber:po.poNumber,vendorId:po.vendorId,vendorName:po.vendorName,receiptDate:opts.receiptDate||postDate,postDate,postPeriod:pp,warehouse:po.warehouse,status:'Released',description:opts.description||`Receipt from ${po.id}`,jeReference:''};
  ...
  for(const {l,qty,item} of toReceive){
    ...
    if(isStockItem(item)){ adjustInventoryBalance({itemId:item.code,warehouse:l.warehouse,location:l.location,qtyIn:qty,unitCost:l.unitCost}); createInvAudit({transactionType:'PO Receipt',referenceNumber:receipt.id,sourceModule:'Purchase Orders',sourceReference:po.id,itemId:item.code,warehouse:l.warehouse,location:l.location,quantityIn:qty,unitCost:l.unitCost,postDate,postPeriod:pp}); const sol=salesOrderLines.find(s=>s.salesOrderId===l.linkedSalesOrder&&s.id===l.linkedSalesOrderLine); if(sol){ const alloc=Math.min(qty,Number(sol.qtyBackordered||0)); sol.qtyBackordered-=alloc; sol.qtyAllocated=Number(sol.qtyAllocated||0)+alloc; inventoryAllocations.push({id:`ALLOC-${inventoryAllocations.length+1001}`,salesOrderId:sol.salesOrderId,salesOrderLineId:sol.id,itemId:item.code,warehouse:l.warehouse,qtyAvailable:qty,qtyAllocated:alloc,qtyBackordered:sol.qtyBackordered,status:'Allocated from PO'}); } }
  }
  receipt.jeReference=createPostedJournal({module:'Inventory',description:`PO Receipt ${receipt.id}`,postPeriod:pp,transactionDate:postDate,sourceRef:receipt.id,lines:jeLines});
...
function serializeApDoc(doc){ return {...doc,billApprovalStatus:billApprovalStatus(doc),paymentApprovalStatus:doc.paymentApprovalStatus||'Not Required',approvalRequired:apApprovalRequired(doc),approvalExceptions:apBillExceptions(doc),threeWayMatched:isThreeWayMatchedWithinTolerance(doc),approvalAudit:workflowAuditLog.filter(a=>a.billId===doc.id)}; }
...
function serializeShipment(sh){
  ...
  const invTx=inventoryTransactions.filter(t=>t.referenceNumber===sh.id||t.referenceNumber===sh.shipmentNumber||t.id===sh.id||t.sourceReference===sh.id||t.sourceReference===sh.shipmentNumber);
  ...
  return {...sh,lines,invoices,inventoryTransactions:invTx,financialDetails};
}
...
const server=http.createServer(async(req,res)=>{const {pathname,query}=parse(req.url,true); const method=req.method||'GET'; try{
 ...
 if(method==='GET'&&pathname==='/api/purchase-orders/receipts') return json(res,200,purchaseReceipts.map(r=>{ const lines=purchaseReceiptLines.filter(l=>l.receiptId===r.id); const linkedBills=poBillLinks.filter(b=>b.receiptId===r.id); const financialDetails=journalEntries.filter(j=>j.sourceRef===r.id||j.jeNumber===r.jeReference); const invTx=inventoryTransactions.filter(t=>t.referenceNumber===r.id||t.sourceReference===r.id||t.id===r.id); return {...r,total:lines.reduce((t,l)=>t+Number(l.extendedCost||0),0),qtyReceived:lines.reduce((t,l)=>t+Number(l.receiptQty||0),0),qtyBilled:lines.reduce((t,l)=>t+Number(l.qtyBilled||0),0),lines,linkedBills,financialDetails,inventoryTransactions:invTx}; }));
 ...
 if(method==='GET'&&pathname.startsWith('/api/purchase-orders/receipts/')){ const id=pathname.split('/').pop(); const r=purchaseReceipts.find(x=>x.id===id||x.receiptNumber===id); if(!r) return json(res,404,{error:'Receipt not found'}); const lines=purchaseReceiptLines.filter(l=>l.receiptId===r.id); return json(res,200,{...r,total:lines.reduce((t,l)=>t+Number(l.extendedCost||0),0),qtyReceived:lines.reduce((t,l)=>t+Number(l.receiptQty||0),0),qtyBilled:lines.reduce((t,l)=>t+Number(l.qtyBilled||0),0),lines,linkedBills:poBillLinks.filter(b=>b.receiptId===r.id),financialDetails:journalEntries.filter(j=>j.sourceRef===r.id||j.jeNumber===r.jeReference),inventoryTransactions:inventoryTransactions.filter(t=>t.referenceNumber===r.id||t.sourceReference===r.id||t.id===r.id)}); }
