...
function postJournalTransactionSafely(je,{userId='admin',requestId=''}={}){
  ...
  try{
    ...
    return{journal:je,alreadyPosted:false};
...
if(method==='POST'&&pathname==='/api/ar/documents/post'){
 ...
 if(method==='POST'&&pathname==='/api/finance/journal-transactions'){ const b=await body(req); if(!b.transactionDate||Number.isNaN(Date.parse(b.transactionDate)))return json(res,400,{error:'A valid transaction date is required'}); const lines=b.lines||[]; const dr=lines.reduce((s,l)=>s+Number(l.debit||0),0); const cr=lines.reduce((s,l)=>s+Number(l.credit||0),0); if(dr!==cr) return json(res,400,{error:'Total debits must equal total credits'}); if(dr<=0)return json(res,400,{error:'Journal entry must contain a debit and credit greater than zero'}); const postDate=b.postDate||b.transactionDate||new Date().toISOString().slice(0,10); const pp=periodFromDate(postDate); validatePeriodOpenForSave('GL',pp); const jeNumber=`JE${String(journalEntries.length+1).padStart(6,'0')}`; const je={jeNumber,module:'GL',description:b.description||'',financialPeriod:pp,postPeriod:pp,transactionDate:postDate,status:'Saved',sourceRef:jeNumber,createdBy:'admin',createdDate:new Date().toISOString(),lines:lines.map(l=>{ const account=requireManualAccount(l.account,'Journal line account'); return normalizeManualJournalLine({...l,account},{jeNumber,branchName:(branchMaster.find(b=>b.code===String(l.branch||'100'))?.name)||'Custom Branch'}); })}; journalEntries.push(je); return json(res,201,je); }
 if(method==='PUT'&&pathname.startsWith('/api/finance/journal-transactions/')){const id=pathname.split('/').pop();const je=journalEntries.find(j=>j.jeNumber===id);if(!je)return json(res,404,{error:'JE not found'});if(!['Saved','Draft'].includes(je.status))return json(res,400,{error:'Only Saved or Draft journal entries can be edited'});const b=await body(req);const isManual=je.module==='GL'&&!je.reversalOf&&!je.reclassOf&&(!je.sourceRef||je.sourceRef===je.jeNumber);const lines=(b.lines||je.lines).map(l=>{const account=isManual?requireManualAccount(l.account,'Journal line account'):requireAccount(l.account,'Journal line account');return isManual?normalizeManualJournalLine({...l,account},{jeNumber:je.jeNumber,branchName:l.branchName||'Chicago HQ'}):{...l,account,debit:Number(l.debit||0),credit:Number(l.credit||0),lineDescription:String(l.lineDescription||l.description||'').slice(0,255),description:String(l.lineDescription||l.description||'').slice(0,255),sourceReference:l.sourceReference||je.sourceRef||''};});const debit=lines.reduce((t,l)=>t+l.debit,0),credit=lines.reduce((t,l)=>t+l.credit,0);if(!lines.length)return json(res,400,{error:'At least one journal line is required'});if(debit!==credit)return json(res,400,{error:'Total debits must equal total credits'});if(debit<=0)return json(res,400,{error:'Journal entry must contain a debit and credit greater than zero'});je.lines=lines;if(b.description!==undefined)je.description=b.description;return json(res,200,je); }
