...
function isEligibleSalesOrderForPayment(order,customerId){ recalcSo(order); return order.customerId===customerId&&['Open','Partially Shipped','Shipped','Pending Invoice'].includes(order.status)&&!['Closed','Cancelled','Voided'].includes(order.status)&&Number(order.openBalance||0)>0; }
...
const server=http.createServer(async(req,res)=>{const {pathname,query}=parse(req.url,true); const method=req.method||'GET'; try{
 ...
 if(method==='GET'&&pathname==='/api/ar/open-sales-orders'){
  ...
  const rows=salesOrders.map(o=>serializeSo(o)).filter(o=>isEligibleSalesOrderForPayment(o,customerId)).map(o=>({
    id:o.id,salesOrderId:o.id,orderNumber:o.orderNumber,customerId:o.customerId,customerName:o.customerName,orderDate:o.orderDate,requestedShipDate:o.requestedShipDate,status:o.status,
