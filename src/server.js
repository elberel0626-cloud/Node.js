...
const server=http.createServer(async(req,res)=>{const {pathname,query}=parse(req.url,true); const method=req.method||'GET'; try{
 ...
 if(method==='POST'&&pathname==='/api/ap/incoming-vendor-bills'){ const b=await body(req); return json(res,201,documentRecognitionEngine.ProcessDocument(b)); }
 if(method==='POST'&&pathname==='/api/ap/incoming-vendor-bills/email'){ const b=await body(req); return json(res,201,documentRecognitionEngine.ProcessDocument({...b,source:'Email Inbox',fileName:b.attachmentName||'email-invoice.pdf'})); }
