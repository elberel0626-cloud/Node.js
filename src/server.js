...
const server=http.createServer(async(req,res)=>{const {pathname,query}=parse(req.url,true); const method=req.method||'GET'; try{
 ...
 if(method==='DELETE'&&pathname.startsWith('/api/ar/documents/')){ const id=pathname.split('/').pop(); const idx=arDocuments.findIndex(x=>x.id===id); if(idx<0)return json(res,404,{error:'Not found'}); const d=arDocuments[idx]; if(['Posted','Open','Closed','Voided'].includes(d.status)) return json(res,400,{error:'Posted transactions cannot be deleted. Please void the transaction instead.'}); const hasApps=(d.applications&&d.applications.length>0)||arDocuments.some(x=>(x.applications||[]).some(a=>a.invoiceId===id||a.paymentId===id||a.reference===id))||paymentApplications.some(a=>a.paymentId===id||a.appliedDocumentId===id); if(hasApps) return json(res,400,{error:'This transaction has applications and cannot be deleted. Please void it instead.'}); arDocuments.splice(idx,1); return json(res,200,{ok:true}); }
 if(method==='POST'&&pathname==='/api/ar/documents/post'){ const {id}=await body(req); const d=arDocuments.find(x=>x.id===id); if(!d)return json(res,404,{error:'Not found'}); if(d.status==='Voided') return json(res,400,{error:'Voided doc'}); if(d.status!=='Saved') return json(res,400,{error:'Only Saved transactions can be posted'}); if(d.status!=='Posted'){ d.posted=true; if(d.reversalOf){ const orig=arDocuments.find(x=>x.id===d.reversalOf); if(orig) orig.status='Voided'; postJE(orig||d,true); } else { postJE(d,false); } if(d.type==='Payment'){ let totalApplied=0; for(const a of d.applications||[]){const inv=arDocuments.find(x=>x.id===a.invoiceId); if(inv){ inv.balance-=Number(a.amount); totalApplied+=Number(a.amount); inv.applications=inv.applications||[]; inv.applications.push({reference:d.id,paymentId:d.id,amount:Number(a.amount),date:d.date,status:'Applied',type:'Payment',appliedFromReference:inv.id,remainingBalance:inv.balance}); inv.status=inv.balance<=0?'Closed':'Open'; } } d.unappliedBalance=Number(d.amount||0)-totalApplied; d.status=d.unappliedBalance===0?'Closed':'Open'; paymentApplications.filter(pa=>pa.paymentId===d.id).forEach(pa=>{pa.status='Applied'; const inv=arDocuments.find(x=>x.id===pa.appliedDocumentId); pa.invoiceOpenBalanceAfter=Number(inv?.balance||pa.invoiceOpenBalanceAfter);}); } else { d.status=Number(d.balance||d.amount||0)>0?'Open':'Closed'; } } return json(res,200,d); }
 if(method==='POST'&&pathname==='/api/ar/documents/void'){ const {id}=await body(req); const d=arDocuments.find(x=>x.id===id); if(!d)return json(res,404,{error:'Not found'}); if(!['Posted','Open','Closed'].includes(d.status)) return json(res,400,{error:'Only posted/open/closed docs can be voided'}); if(d.type==='Payment'){ const apps=paymentApplications.filter(a=>a.paymentId===d.id&&a.status==='Posted'); for(const a of apps){ const inv=arDocuments.find(x=>x.id===a.appliedDocumentId); if(inv&&inv.status!=='Voided'){ inv.balance+=Number(a.appliedAmount||0); inv.status=inv.balance===0?'Closed':'Open'; inv.applications=inv.applications||[]; inv.applications.push({reference:`REV-${d.id}`,paymentId:d.id,amount:-Number(a.appliedAmount||0),date:new Date().toISOString().slice(0,10),status:'Reversed',type:'Payment Reversal',appliedFromReference:inv.id,remainingBalance:inv.balance}); } a.status='Reversed'; paymentApplications.push({...a,applicationId:`APP-${String(applicationSeq++).padStart(6,'0')}`,status:'Reversed',reversalReference:`REV-${d.id}`,reversalAmount:Number(a.appliedAmount||0),applicationDate:new Date().toISOString().slice(0,10)}); } d.status='Voided'; d.unappliedBalance=0; postJE(d,true); return json(res,200,{message:'Payment voided and applications reversed',document:d}); }
 const prefix=`VOID-${d.id}`; const rev={...d,id:`${prefix}-${String(arDocuments.filter(x=>x.id.startsWith(prefix)).length+1).padStart(3,'0')}`,status:'Saved',posted:false,reversalOf:d.id,createdDate:new Date().toISOString().slice(0,10)}; arDocuments.push(rev); return json(res,201,{message:'Reversal draft created',reversal:rev}); }
 if(method==='POST'&&pathname==='/api/ar/release/post-selected'){ const b=await body(req); const ids=b.ids||[]; const updated=[]; for(const id of ids){ const d=arDocuments.find(x=>x.id===id); if(d&&d.status==='Saved'){ d.posted=true; postJE(d,false); updated.push(d);} } return json(res,200,{posted:updated.length,documents:updated}); }
