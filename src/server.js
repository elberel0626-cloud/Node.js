...
const PO_STATUSES = ['Draft','Saved','Pending Approval','Approved','Open','Partially Received','Received','Partially Billed','Billed','Closed','Cancelled','Voided','Rejected'];
...
const poBillLinks = [];
const vendorItems = [];
const poSettings = { approvalRequired: false, autoApplyPrepayments: true, quantityTolerancePct: 5, priceTolerancePct: 5, amountTolerance: 100, allowMatchOverride: false };
...
function refreshPoStatus(po, action='Refresh Status', note=''){
  ...
  if(['Draft','Saved','Pending Approval','Approved','Cancelled','Voided','Rejected'].includes(po.status)&&!['Open','Partially Received','Received','Partially Billed','Billed','Closed'].includes(po.status)) return po;
  const ordered=lines.reduce((s,l)=>s+Number(l.qtyOrdered||0),0);
  const received=lines.reduce((s,l)=>s+Number(l.qtyReceived||0),0);
  const billed=lines.reduce((s,l)=>s+Number(l.qtyBilled||0),0);
  let next='Open';
  if(ordered>0&&received>=ordered&&billed>=ordered) next='Closed';
  else if(ordered>0&&billed>=ordered) next='Billed';
  else if(billed>0) next='Partially Billed';
  else if(ordered>0&&received>=ordered) next='Received';
  else if(received>0) next='Partially Received';
  if(po.status!==next) setPoStatus(po,next,action,note);
  return recalcPo(po);
}
function poApprovalMatches(po){
  recalcPo(po);
  const amount=Number(po.poTotal||0);
  const vendor=poVendor(po.vendorId)||{};
  const lines=purchaseOrderLines.filter(l=>l.poId===po.id);
  return approvalRules.filter(r=>r.active!==false&&(r.module==='PO'||String(r.ruleId||'').startsWith('POAPR'))).filter(r=>{
    const from=Number(r.amountFrom||0), to=r.amountTo==null?Infinity:Number(r.amountTo);
    const amountOk=amount>=from&&amount<=to;
    const vendorOk=!r.vendor||r.vendor==='Any'||r.vendor===po.vendorId;
    const vendorClassOk=!r.vendorClass||r.vendorClass==='Any'||(r.vendorClass==='New'&&(vendor.isNew||vendor.createdDate||'').slice(0,10)>='2026-01-01')||r.vendorClass===vendor.vendorClass;
    const accountOk=!r.expenseAccount||r.expenseAccount==='Any'||(r.expenseAccount==='Capital Asset'?lines.some(l=>String(l.expenseAccount||l.inventoryAccount||'').startsWith('15')):lines.some(l=>[l.expenseAccount,l.inventoryAccount].includes(r.expenseAccount)));
    return amountOk&&vendorOk&&vendorClassOk&&accountOk;
  }).sort((a,b)=>Number(a.priority||0)-Number(b.priority||0));
}
function poRequiresApproval(po){ const rules=poApprovalMatches(po); return rules.some(r=>!r.noApproval&&Number(r.approvalLevel||0)>0); }
function openPoIfNoApproval(po){ if(poRequiresApproval(po)){ po.approvalRequired=true; return false; } po.approvalRequired=false; if(['Saved','Approved'].includes(po.status)) setPoStatus(po,'Open','Open PO','No PO approval rule matched.'); return true; }
...
function seedPurchaseOrders(){
  ...
  const defs=[{id:'PO-1001',vendorId:'VEND-1004',orderDate:'2026-05-01',status:'Open',terms:'50% Prepayment, Net 30',prepaymentRequired:7000,lines:[['ITEM-1001',100,80],['ITEM-1002',75,80]]},{id:'PO-1002',vendorId:'VEND-1002',orderDate:'2026-05-03',status:'Partially Received',lines:[['ITEM-1007',10,420],['ITEM-1009',5,240]]},{id:'PO-1003',vendorId:'VEND-1003',orderDate:'2026-05-05',status:'Saved',lines:[['ITEM-1010',20,65]]},{id:'PO-1004',vendorId:'VEND-1001',orderDate:'2026-05-07',status:'Pending Approval',lines:[['ITEM-1008',50,55]]},{id:'PO-1005',vendorId:'VEND-1005',orderDate:'2026-05-08',status:'Open',lines:[['ITEM-1006',1,500]]}];
  for(const d of defs){ const v=poVendor(d.vendorId); const po={id:d.id,poType:'RO',poNumber:d.id,status:d.status,orderDate:d.orderDate,requestedDate:d.orderDate,promisedDate:'2026-05-15',postDate:d.orderDate,postPeriod:d.orderDate.slice(0,7),vendorId:d.vendorId,vendorName:v?.name||'',vendorLocation:'MAIN',branch:'100',warehouse:v?.defaultWarehouse||'MAIN',currency:'USD',terms:d.terms||v?.terms||'NET30',buyer:'BUYER-01',shipVia:'Best Way',fobPoint:'Destination',vendorRef:'',description:`Sample ${d.id}`,freight:0,prepaymentRequired:Number(d.prepaymentRequired||0),approvalRequired:d.status==='Pending Approval'}; purchaseOrders.push(po); d.lines.forEach((l,i)=>purchaseOrderLines.push(calcPoLine({itemId:l[0],qtyOrdered:l[1],unitCost:l[2],qtyReceived:d.id==='PO-1002'&&i===0?5:0},i,po.id))); recalcPo(po); }
...
function createReceiptFromPo(poId, opts={}){
  ...
  if(!['Open','Partially Received','Approved','Partially Billed'].includes(po.status)) throw new Error('Create Receipt enabled for Open / Partially Received POs.');
...
function createBillFromPo(poId, opts={}){
  const po=purchaseOrders.find(p=>p.id===poId||p.poNumber===poId); if(!po) throw new Error('PO not found');
  const vendor=poVendor(po.vendorId); if(vendor?.status==='Inactive') throw new Error('Cannot use inactive vendor.');
  const postDate=opts.postDate||new Date().toISOString().slice(0,10); const pp=validatePoPeriods('Bill',postDate);
  const receiptFilter=opts.receiptId||opts.receiptNumber||'';
  let sourceLines=[];
  if(receiptFilter){ const r=purchaseReceipts.find(x=>x.id===receiptFilter||x.receiptNumber===receiptFilter); if(!r) throw new Error('Receipt not found'); if(r.status==='Voided') throw new Error('Cannot bill a voided receipt.'); sourceLines=receiptUnbilledLines(r.id).map(rl=>({poLine:purchaseOrderLines.find(l=>l.id===rl.poLineId), receiptLine:rl, qty:rl.qtyRemaining})).filter(x=>x.poLine&&x.qty>0); }
  else sourceLines=purchaseOrderLines.filter(l=>l.poId===po.id&&((Number(l.qtyReceived||0)>Number(l.qtyBilled||0))||(!isStockItem(itemMaster.find(i=>i.code===l.inventoryId))&&Number(l.qtyOrdered||0)>Number(l.qtyBilled||0)))).map(l=>({poLine:l,receiptLine:null,qty:Math.max(0,(Number(l.qtyReceived||l.qtyOrdered||0)-Number(l.qtyBilled||0)))}));
  if(!sourceLines.length) throw new Error('No received quantities are available for billing.');
  if(apDocuments.some(d=>d.type==='Bill'&&d.vendorId===po.vendorId&&String(d.vendorRef||'').toLowerCase()===String(opts.vendorRef||'').toLowerCase()&&opts.vendorRef)) throw new Error('Duplicate vendor invoice number.');
  const amount=sourceLines.reduce((s,x)=>s+Number(x.qty||0)*Number(x.poLine.unitCost||0),0); const billId=opts.billId||nextPoId('BILL-PO',apDocuments);
  const availablePrepay=poPrepayments.filter(p=>p.poId===po.id&&['Open','Partially Applied'].includes(p.status)&&Number(p.remainingBalance||p.amount-p.appliedAmount)>0); let applyAmt=poSettings.autoApplyPrepayments?Math.min(amount,availablePrepay.reduce((s,p)=>s+Number(p.remainingBalance||p.amount-p.appliedAmount),0)):0;
  const bill={id:billId,type:'Bill',vendorId:po.vendorId,vendorName:po.vendorName,date:postDate,postDate,postPeriod:pp,dueDate:opts.dueDate||postDate,terms:po.terms,status:'Open',posted:true,amount,balance:amount-applyAmt,vendorRef:opts.vendorRef||billId,invoiceNumber:opts.vendorRef||billId,sourcePoId:po.id,sourceReceiptId:receiptFilter,prepaymentApplied:applyAmt,lines:sourceLines.map(x=>({poId:po.id,receiptId:x.receiptLine?.receiptId||receiptFilter,receiptNumber:x.receiptLine?.receiptId||receiptFilter,inventoryId:x.poLine.inventoryId,description:x.poLine.description,qty:x.qty,qtyReceived:x.receiptLine?.qtyReceived||x.poLine.qtyReceived,qtyBilled:x.poLine.qtyBilled,qtyRemaining:x.qty,unitCost:x.poLine.unitCost,lineTotal:x.qty*x.poLine.unitCost,expenseAccount:x.poLine.inventoryAccount||x.poLine.expenseAccount,rniAccount:x.poLine.apAccrualAccount}))};
  const jeLines=[{account:POSTING_ACCOUNTS.poRni,debit:amount},{account:POSTING_ACCOUNTS.apTrade,credit:amount}]; for(const p of availablePrepay){ if(applyAmt<=0) break; const a=Math.min(applyAmt,Number(p.remainingBalance||p.amount-p.appliedAmount)); p.appliedAmount=Number(p.appliedAmount||0)+a; p.remainingBalance=Number(p.amount||0)-p.appliedAmount; p.status=p.remainingBalance<=0?'Closed':'Partially Applied'; poPrepaymentApplications.push({id:`PPA-${poPrepaymentApplications.length+1001}`,prepaymentId:p.id,poId:po.id,billId,applicationDate:postDate,appliedAmount:a,jeReference:''}); jeLines.push({account:POSTING_ACCOUNTS.apTrade,debit:a,sourceReference:p.id},{account:POSTING_ACCOUNTS.vendorDeposit,credit:a,sourceReference:p.id}); applyAmt-=a; }
  const jeReference=createPostedJournal({module:'AP',description:`AP Bill from PO ${billId}`,postPeriod:pp,transactionDate:postDate,sourceRef:billId,lines:jeLines}); poPrepaymentApplications.filter(a=>a.billId===billId).forEach(a=>a.jeReference=jeReference); apDocuments.push(bill); poBillLinks.push({id:`POBL-${poBillLinks.length+1001}`,poId:po.id,receiptId:receiptFilter,billId,billAmount:amount,prepaymentApplied:bill.prepaymentApplied,remainingDue:bill.balance,status:bill.balance?'Open':'Closed',jeReference}); sourceLines.forEach(x=>{ if(Number(x.qty||0)>Math.max(Number(x.poLine.qtyReceived||x.poLine.qtyOrdered||0)-Number(x.poLine.qtyBilled||0),0)) throw new Error('Cannot bill more than received quantity.'); x.poLine.qtyBilled=Number(x.poLine.qtyBilled||0)+Number(x.qty||0); if(x.receiptLine){ const rl=purchaseReceiptLines.find(l=>l.id===x.receiptLine.id); if(rl) rl.qtyBilled=Number(rl.qtyBilled||0)+Number(x.qty||0); } });
  refreshPoStatus(po,'Create Bill',billId); return bill;
}
function createPoPrepayment(poId, opts={}){ const po=purchaseOrders.find(p=>p.id===poId); if(!po) throw new Error('PO not found'); if(!['Saved','Approved','Open'].includes(po.status)) throw new Error('Create Prepayment enabled for Saved, Approved, Open POs.'); const postDate=opts.postDate||new Date().toISOString().slice(0,10); const pp=validatePoPeriods('Prepayment',postDate); const amount=Number(opts.amount||(opts.percent?po.poTotal*Number(opts.percent)/100:0)); if(amount<=0) throw new Error('Prepayment amount is required.'); const id=opts.id||nextPoId('PREPAY',poPrepayments); const jeReference=createPostedJournal({module:'AP',description:`PO vendor prepayment ${id}`,postPeriod:pp,transactionDate:postDate,sourceRef:id,lines:[{account:POSTING_ACCOUNTS.vendorDeposit,debit:amount},{account:opts.cashAccount||POSTING_ACCOUNTS.apCash,credit:amount}]}); const p={id,prepaymentNumber:id,vendorId:po.vendorId,vendorName:po.vendorName,poId:po.id,poNumber:po.poNumber,paymentDate:opts.paymentDate||postDate,postDate,postPeriod:pp,paymentMethod:opts.paymentMethod||'Check',cashAccount:opts.cashAccount||POSTING_ACCOUNTS.apCash,amount,appliedAmount:0,remainingBalance:amount,status:'Open',description:opts.description||'',jeReference}; poPrepayments.push(p); recalcPo(po); return p; }
seedPurchaseOrders();
...
const server=http.createServer(async(req,res)=>{const {pathname,query}=parse(req.url,true); const method=req.method||'GET'; try{
 ...
 if(method==='GET'&&pathname==='/api/purchase-orders/setup'){ return json(res,200,{settings:poSettings,types:purchaseOrderTypes,buyers,landedCostCodes,vendorItems,approvalRules}); }
 ...
 if(method==='GET'&&pathname==='/api/purchase-orders/lookup'){
  ...
  return json(res,200,rows.map(p=>({poId:p.id,poNumber:p.poNumber,vendorId:p.vendorId,vendorName:p.vendorName,vendor:`${p.vendorId} - ${p.vendorName}`,date:p.orderDate,total:p.poTotal,receivedAmount:p.receivedAmount,billedAmount:p.billedAmount,openAmount:p.openPoBalance,status:p.status,lines:receiptLinesForPo(p)})));
 }
 ...
 if(method==='POST'&&pathname==='/api/purchase-orders/action'){ const b=await body(req); const po=purchaseOrders.find(p=>p.id===b.id||p.poNumber===b.id); if(!po) return json(res,404,{error:'Purchase order not found'}); if(b.action==='Submit for Approval'){ const rules=poApprovalMatches(po).filter(r=>!r.noApproval&&Number(r.approvalLevel||0)>0); if(!rules.length){ po.approvalRequired=false; setPoStatus(po,'Open','Submit for Approval','No PO approval rule matched.'); } else { po.approvalRequired=true; setPoStatus(po,'Pending Approval','Submit for Approval'); rules.forEach(r=>poApprovals.push({id:`POA-${poApprovals.length+1001}`,poId:po.id,approvalLevel:Number(r.approvalLevel||1),assignedTo:r.approver,status:'Pending',comments:'',dateAssigned:new Date().toISOString(),approvedDate:'',rejectedDate:'',ruleId:r.ruleId})); } } else if(b.action==='Approve'){ setPoStatus(po,'Open','Approve','Approved POs move to Open.'); poApprovals.filter(a=>a.poId===po.id&&a.status==='Pending').forEach(a=>{a.status='Approved';a.approvedDate=new Date().toISOString();a.comments=b.comments||'';}); } else if(b.action==='Reject'){ setPoStatus(po,'Rejected','Reject',b.comments||''); poApprovals.filter(a=>a.poId===po.id&&a.status==='Pending').forEach(a=>{a.status='Rejected';a.rejectedDate=new Date().toISOString();a.comments=b.comments||'';}); } else if(b.action==='Open PO'){ if(po.approvalRequired&&po.status!=='Approved') return json(res,400,{error:'Open PO enabled after approval or if approval is not required.'}); setPoStatus(po,'Open','Open PO','No GL or inventory impact.'); } else if(b.action==='Cancel PO'){ if(['Received','Billed','Closed'].includes(po.status)) return json(res,400,{error:'Cancel only allowed if not fully received/billed.'}); setPoStatus(po,'Cancelled','Cancel PO'); } else return json(res,400,{error:'Invalid PO action'}); return json(res,200,serializePo(po)); }
 ...
 if(method==='POST'&&pathname==='/api/purchase-orders/create-bill'){ const b=await body(req); return json(res,201,createBillFromPo(b.poId||b.id,b)); }
 if(method==='POST'&&pathname==='/api/purchase-orders/create-prepayment'){ const b=await body(req); return json(res,201,createPoPrepayment(b.poId||b.id,b)); }
