...
const server=http.createServer(async(req,res)=>{const {pathname,query}=parse(req.url,true); const method=req.method||'GET'; try{
 ...
 if(method==='POST'&&pathname==='/api/ar/customers'){ const b=await body(req); if(!b.id) return json(res,400,{error:'Customer ID required'}); if(!b.name) return json(res,400,{error:'Customer Name required'}); if(customers.find(c=>c.id===b.id)) return json(res,400,{error:'Customer ID must be unique'}); const c={id:b.id,name:b.name,status:b.status||'Active',billingAddress:b.billingAddress||'',shippingAddress:b.shippingAddress||'',phone:b.phone||'',email:b.email||'',terms:b.terms||'NET30',taxZone:b.taxZone||'DEFAULT',currency:b.currency||'USD',contactPerson:b.contactPerson||''}; customers.push(c); return json(res,201,c); }
 ...
 if(doc.type==='Payment'){ for(const app of doc.applications||[]){ const inv=arDocuments.find(d=>d.id===app.invoiceId); paymentApplications.push({applicationId:`APP-${String(applicationSeq++).padStart(6,'0')}`,paymentRef:doc.id,paymentId:doc.id,customerId:doc.customerId,appliedDocumentType:'Invoice',appliedDocumentRef:inv?.id||app.invoiceId,appliedDocumentId:inv?.id||app.invoiceId,applicationDate:doc.date,invoiceOriginalAmount:Number(inv?.amount||0),invoiceOpenBalanceBefore:Number(inv?.balance||0),appliedAmount:Number(app.amount||0),invoiceOpenBalanceAfter:Number((inv?.balance||0)-Number(app.amount||0)),status:'Saved'}); } } arDocuments.push(doc); return json(res,201,doc);} 
 ...
 if(['Invoice','Credit Memo','Debit Memo'].includes(d.type)){
  ...
  for(const app of originalApps){
   ...
   if(pay&&pay.status!=='Voided'){
    ...
    paymentApplications.push({applicationId:`APP-${String(applicationSeq++).padStart(6,'0')}`,paymentRef:pay.id,paymentId:pay.id,customerId:pay.customerId,appliedDocumentType:d.type,appliedDocumentRef:d.id,appliedDocumentId:d.id,applicationDate:new Date().toISOString().slice(0,10),invoiceOriginalAmount:toNumber(d.amount),invoiceOpenBalanceBefore:toNumber(d.balance),appliedAmount:amt,invoiceOpenBalanceAfter:toNumber(d.balance),status:'Reversed',reversalReference:`REV-${d.id}`,reversalAmount:amt});
 ...
 if(method==='POST'&&pathname==='/api/ar/payments/apply'){ const {paymentId,applications=[]}=await body(req); const pay=arDocuments.find(x=>x.id===paymentId&&x.type==='Payment'); if(!pay) return json(res,404,{error:'Payment not found'}); normalizeArStatus(pay); if(pay.status!=='Open') return json(res,400,{error:'Only open payments can be applied'}); let rem=toNumber(pay.unappliedBalance); for(const app of applications){ const amt=toNumber(app.amount); const doc=arDocuments.find(x=>x.id===app.documentId&&x.customerId===pay.customerId&&(x.type==='Invoice'||x.type==='Debit Memo')); if(!doc||doc.status==='Voided') return json(res,400,{error:'Invalid application target'}); if(amt<=0) continue; if(amt>rem||amt>toNumber(doc.balance)) return json(res,400,{error:'Invalid application amount'}); const before=toNumber(doc.balance); doc.balance=before-amt; rem-=amt; doc.applications=doc.applications||[]; doc.applications.push({reference:pay.id,paymentId:pay.id,amount:amt,date:new Date().toISOString().slice(0,10),status:'Applied',type:'Payment',appliedFromReference:doc.id,remainingBalance:doc.balance}); paymentApplications.push({applicationId:`APP-${String(applicationSeq++).padStart(6,'0')}`,paymentRef:pay.id,paymentId:pay.id,customerId:pay.customerId,appliedDocumentType:doc.type,appliedDocumentRef:doc.id,appliedDocumentId:doc.id,applicationDate:new Date().toISOString().slice(0,10),invoiceOriginalAmount:toNumber(doc.amount),invoiceOpenBalanceBefore:before,appliedAmount:amt,invoiceOpenBalanceAfter:toNumber(doc.balance),status:'Applied'}); normalizeArStatus(doc); }
