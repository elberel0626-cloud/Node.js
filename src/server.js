...
const server=http.createServer(async(req,res)=>{const {pathname,query}=parse(req.url,true); const method=req.method||'GET'; try{
 ...
 const lines=(b.lines||[]).map(l=>{ const item=itemMaster.find(i=>i.code===l.itemCode)||{}; const qty=Number(l.qty||0); const unitPrice=Number(l.unitPrice??item.salesPrice??0); const discountPct=Number(l.discountPct||0); const base=qty*unitPrice; const discount=base*(discountPct/100); const isMemo=(b.type==='Credit Memo'||b.type==='Debit Memo'); const taxable=isMemo?0:((l.taxable??item.taxable)?1:0); const tax=isMemo?0:(taxable?(base-discount)*0.1:0); const lineTotal=base-discount+tax; return {itemCode:l.itemCode,description:l.description||item.name||'',qty,unitPrice,discountPct,taxable:!!taxable,tax,lineTotal,cost:Number(item.cost||0)}; });
 ...
 if(method==='POST'&&pathname==='/api/ar/documents/post'){ const {id}=await body(req); const d=arDocuments.find(x=>x.id===id); if(!d)return json(res,404,{error:'Not found'}); if(d.status==='Voided') return json(res,400,{error:'Voided doc'}); if(d.status!=='Posted'){ d.status='Posted'; d.posted=true; if(d.reversalOf){ const orig=arDocuments.find(x=>x.id===d.reversalOf); if(orig) orig.status='Voided'; postJE(orig||d,true); } else { postJE(d,false); } if(d.type==='Payment'){ for(const a of d.applications||[]){const inv=arDocuments.find(x=>x.id===a.invoiceId); if(inv){ inv.balance-=Number(a.amount); inv.applications=inv.applications||[]; inv.applications.push({reference:d.id,paymentId:d.id,amount:Number(a.amount),date:d.date,status:'Posted',type:'Payment',appliedFromReference:inv.id,remainingBalance:inv.balance}); inv.status=inv.balance<=0?'Closed':'Open'; } } paymentApplications.filter(pa=>pa.paymentId===d.id).forEach(pa=>{pa.status='Posted'; const inv=arDocuments.find(x=>x.id===pa.appliedDocumentId); pa.invoiceOpenBalanceAfter=Number(inv?.balance||pa.invoiceOpenBalanceAfter);}); } } return json(res,200,d); }
 ...
 if(method==='POST'&&pathname==='/api/ar/documents/apply'){ const b=await body(req); const source=arDocuments.find(d=>d.id===b.sourceId); if(!source) return json(res,404,{error:'Source not found'}); if(source.status==='Voided'||source.balance<=0) return json(res,400,{error:'Source not open'}); const isCredit=source.type==='Credit Memo', isDebit=source.type==='Debit Memo', isPayment=source.type==='Payment'; if(!isCredit&&!isDebit&&!isPayment) return json(res,400,{error:'Apply supported for credit/debit memos/payments only'}); const apps=(b.applications||[]).map(a=>({targetId:a.targetId,amount:Number(a.amount||0)})).filter(a=>a.amount>0); let available=Number(source.balance||0); for(const a of apps){ const t=arDocuments.find(d=>d.id===a.targetId); if(!t) return json(res,400,{error:'Target not found'}); if(t.customerId!==source.customerId) return json(res,400,{error:'Same customer only'}); if(['Voided','Closed'].includes(t.status)||Number(t.balance||0)<=0) return json(res,400,{error:'Target not open'}); const allowed=isCredit?['Invoice','Debit Memo'].includes(t.type):(isDebit?['Payment','Credit Memo'].includes(t.type):['Invoice','Debit Memo'].includes(t.type)); if(!allowed) return json(res,400,{error:'Invalid target type'}); if(a.amount>available||a.amount>Number(t.balance||0)) return json(res,400,{error:'Invalid applied amount'}); available-=a.amount; t.balance=Number(t.balance)-a.amount; t.status=t.balance<=0?'Closed':(t.status==='Saved'?'Open':t.status); t.applications=t.applications||[]; t.applications.push({reference:source.id,paymentId:source.id,amount:a.amount,date:b.date||source.date,status:'Posted',type:source.type,targetId:t.id,remainingBalance:t.balance}); source.applications=source.applications||[]; source.applications.push({reference:t.id,paymentId:t.id,amount:a.amount,date:b.date||source.date,status:'Posted',type:t.type,targetId:t.id,remainingBalance:available}); }
 source.balance=available; source.status=source.balance<=0?'Closed':source.status; return json(res,200,source); }
