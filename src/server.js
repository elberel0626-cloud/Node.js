...
function evaluateApprovalRules(doc){
  ...
  const expenseAccounts=new Set((doc.lines||[]).map(l=>String(l.expenseAccount||'')));
  return approvalRules.filter(r=>r.active!==false&&amount>=Number(r.amountFrom||0)&&(r.amountTo==null||amount<=Number(r.amountTo))&&ruleFieldMatches(r.vendor,doc.vendorId)&&ruleFieldMatches(r.branch,doc.branch)&&ruleFieldMatches(r.department,doc.department)&&(!r.expenseAccount||r.expenseAccount==='Any'||expenseAccounts.has(String(r.expenseAccount)))).sort((a,b)=>Number(a.approvalLevel||1)-Number(b.approvalLevel||1)||Number(a.priority||0)-Number(b.priority||0));
}
function apApprovalRequired(doc){ return doc.type==='Bill'&&evaluateApprovalRules(doc).length>0; }
function serializeApDoc(doc){ return {...doc,approvalRequired:apApprovalRequired(doc),approvalAudit:workflowAuditLog.filter(a=>a.billId===doc.id)}; }
...
function approvalQueueRows(view='All',userId='admin'){
  return apDocuments.filter(d=>d.type==='Bill'&&['Pending Approval','Approved','Rejected','Information Requested'].includes(d.status)).map(d=>({billNumber:d.id,vendor:d.vendorName,invoiceNumber:d.vendorRef||d.invoiceNumber||'',billDate:d.date,amount:Number(d.amount||0),dueDate:d.dueDate,currentApprover:currentApproverLabel(d),approvalStatus:d.status,daysWaiting:daysWaiting(d),priority:Math.min(...(d.approvals||[]).map(a=>Number(a.priority||99)),99),escalated:(d.approvals||[]).some(a=>a.escalated),department:d.department||'Finance',createdBy:d.createdBy||'ap.clerk',approvals:d.approvals||[]})).filter(r=>{
    ...
    if(view==='Pending Approvals'||view==='All Pending Approvals') return r.approvalStatus==='Pending Approval'||r.approvalStatus==='Information Requested';
...
function submitBillForApproval(doc,{userId='admin',duplicateOverrideReason=''}={}){
  ...
  if(!rules.length){ doc.approvalRequired=false; doc.approvalStatus='Not Required'; addWorkflowAudit({billId:doc.id,action:'Submit - Not Required',userId,fromStatus:old,toStatus:doc.status,comments:'No approval is required for this bill.'}); return doc; }
...
function recallBillApproval(doc,{userId='admin',comments=''}={}){
  ...
  doc.status='Saved'; doc.approvalStatus='Recalled'; addWorkflowAudit({billId:doc.id,action:'Recall',userId,fromStatus:old,toStatus:'Saved',comments});
