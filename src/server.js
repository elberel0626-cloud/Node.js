...
if(method==='POST'&&pathname==='/api/ar/documents/post'){
 ...
 if(method==='POST'&&pathname==='/api/finance/journal-transactions/post'){ const {jeNumber}=await body(req); const je=journalEntries.find(j=>j.jeNumber===jeNumber); if(!je) return json(res,404,{error:'JE not found'}); validatePeriodOpen('GL',je.postPeriod||je.financialPeriod||periodFromDate(je.transactionDate)); if(je.status!=='Saved') return json(res,400,{error:'Only Saved transactions can be posted'}); const dr=je.lines.reduce((s,l)=>s+l.debit,0), cr=je.lines.reduce((s,l)=>s+l.credit,0); if(dr<=0)return json(res,400,{error:'Journal entry must contain a debit and credit greater than zero'}); if(dr!==cr) return json(res,400,{error:'Out-of-balance JE'}); if(je.status!=='Posted'){ je.status='Posted'; je.lines.forEach(l=>{ if(l.debit) bump(l.account,'Debit',l.debit); if(l.credit) bump(l.account,'Credit',l.credit);}); if(je.reversalOf){ const orig=journalEntries.find(x=>x.jeNumber===je.reversalOf); if(orig) orig.status='Reversed'; } } return json(res,200,je); }
