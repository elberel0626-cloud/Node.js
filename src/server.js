...
const poBillLinks = [];
const vendorItems = [];
const incomingApBills = [];
let incomingApBillSeq = 1001;
...
function textScore(a='',b=''){
  ...
  const at=new Set(a.split(/\W+/).filter(Boolean)), bt=new Set(b.split(/\W+/).filter(Boolean));
  const common=[...at].filter(x=>bt.has(x)).length; return Math.round((common/Math.max(at.size,bt.size,1))*90);
}
function moneyValue(value){ const n=Number(String(value||'').replace(/[$,]/g,'')); return Number.isFinite(n)?n:0; }
function normalizeInvoiceDate(value){
  const raw=String(value||'').trim(); if(!raw) return '';
  const iso=raw.match(/(20\d{2}|19\d{2})[-/.](\d{1,2})[-/.](\d{1,2})/); if(iso) return `${iso[1]}-${String(iso[2]).padStart(2,'0')}-${String(iso[3]).padStart(2,'0')}`;
  const us=raw.match(/(\d{1,2})[-/.](\d{1,2})[-/.](\d{2,4})/); if(us){ const y=us[3].length===2?`20${us[3]}`:us[3]; return `${y}-${String(us[1]).padStart(2,'0')}-${String(us[2]).padStart(2,'0')}`; }
  const named=raw.match(/(jan|feb|mar|apr|may|jun|jul|aug|sep|sept|oct|nov|dec)[a-z]*\s+(\d{1,2}),?\s+(\d{2,4})/i); if(named){ const months={jan:'01',feb:'02',mar:'03',apr:'04',may:'05',jun:'06',jul:'07',aug:'08',sep:'09',sept:'09',oct:'10',nov:'11',dec:'12'}; const y=named[3].length===2?`20${named[3]}`:named[3]; return `${y}-${months[named[1].toLowerCase().slice(0,3)]}-${String(named[2]).padStart(2,'0')}`; }
  return raw;
}
function fieldPick(text, patterns){ for(const re of patterns){ const m=String(text).match(re); if(m) return String(m[1]||m[0]).trim().replace(/[ \t]+$/,''); } return ''; }
function allPoNumbers(text){ return [...new Set([...String(text).matchAll(/\b(?:P\.?O\.?|purchase order)(?:\s*(?:number|no\.?|#))?\s*[:#-]?\s*([A-Z]+-?\d[\w-]*|\d[\w-]*)/gi)].map(m=>String(m[1]).toUpperCase().startsWith('PO')?String(m[1]).toUpperCase():`PO-${String(m[1]).toUpperCase().replace(/^[-#]/,'')}`))]; }
function parseIncomingInvoiceText(text=''){
  const t=String(text||'').replace(/\r/g,'');
  const lines=t.split('\n').map(x=>x.trim()).filter(Boolean);
  const vendor_name=fieldPick(t,[/vendor(?:\s+name)?\s*[:#-]\s*(.+)/i,/supplier(?:\s+name)?\s*[:#-]\s*(.+)/i,/remit to\s*[:#-]\s*(.+)/i,/^(.+?)\s+(?:invoice|tax invoice)\b/im]);
  const invoice_number=fieldPick(t,[/invoice\s*(?:number|no\.?|#)\s*[:#-]?\s*([A-Z0-9][\w.-]*)/i,/\binv(?:oice)?\s*(?:number|no\.?|#)?\s*[:#-]\s*([A-Z0-9][\w.-]*)/i,/document\s*(?:number|no\.?)\s*[:#-]?\s*([A-Z0-9][\w.-]*)/i]);
  const invoiceDateRaw=fieldPick(t,[/invoice\s*date\s*[:#-]?\s*([A-Za-z]{3,9}\s+\d{1,2},?\s+\d{2,4}|\d{4}[-/.]\d{1,2}[-/.]\d{1,2}|\d{1,2}[-/.]\d{1,2}[-/.]\d{2,4})/i,/(?:bill|document)\s*date\s*[:#-]?\s*([A-Za-z]{3,9}\s+\d{1,2},?\s+\d{2,4}|\d{4}[-/.]\d{1,2}[-/.]\d{1,2}|\d{1,2}[-/.]\d{1,2}[-/.]\d{2,4})/i,/\bdate\s*[:#-]?\s*([A-Za-z]{3,9}\s+\d{1,2},?\s+\d{2,4}|\d{4}[-/.]\d{1,2}[-/.]\d{1,2}|\d{1,2}[-/.]\d{1,2}[-/.]\d{2,4})/i]);
  const dueDateRaw=fieldPick(t,[/due\s*date\s*[:#-]?\s*([A-Za-z]{3,9}\s+\d{1,2},?\s+\d{2,4}|\d{4}[-/.]\d{1,2}[-/.]\d{1,2}|\d{1,2}[-/.]\d{1,2}[-/.]\d{2,4})/i,/payment\s*due\s*[:#-]?\s*([A-Za-z]{3,9}\s+\d{1,2},?\s+\d{2,4}|\d{4}[-/.]\d{1,2}[-/.]\d{1,2}|\d{1,2}[-/.]\d{1,2}[-/.]\d{2,4})/i]);
  const po_numbers=allPoNumbers(t); const po_number=po_numbers[0]||'';
  const lineItems=[];
  for(const line of lines){
    if(/subtotal|total|tax|freight|shipping|balance|amount due|invoice date|due date|invoice number|purchase order|\bpo\b/i.test(line)) continue;
    const parts=line.includes('|')?line.split('|').map(x=>x.trim()).filter(Boolean):line.split(/\s{2,}|\t|,/).map(x=>x.trim()).filter(Boolean);
    let qty,desc,unit,amount,item_number='',vendor_part_number='',uom='EA';
    if(parts.length>=4&&/^\d+(?:\.\d+)?$/.test(parts[0])&&/[\d,.]+$/.test(parts.at(-1))){ qty=Number(parts[0]); amount=moneyValue(parts.at(-1)); unit=moneyValue(parts.at(-2)); desc=parts.slice(1,-2).join(' '); }
    else {
      const m=line.match(/^\s*(?:(\S+)\s+)?(\d+(?:\.\d+)?)\s+(EA|Each|PCS|HR|HRS|LB|FT|BOX|CS)?\s*(.+?)\s+\$?([\d,]+(?:\.\d{2,4})?)\s+\$?([\d,]+(?:\.\d{2})?)\s*$/i);
      if(m){ item_number=/[A-Za-z]/.test(m[1]||'')?m[1]:''; qty=Number(m[2]); uom=(m[3]||'EA').toUpperCase(); desc=m[4].trim(); unit=moneyValue(m[5]); amount=moneyValue(m[6]); }
    }
    if(qty>0&&desc&&unit>=0&&amount>=0&&!/qty|description|unit|price|amount|item/i.test(desc)) lineItems.push({item_number, vendor_part_number, description:desc, qty, quantity:qty, uom, unitCost:unit, unit_price:unit, extendedCost:amount||qty*unit, amount:amount||qty*unit});
  }
  const subtotal=moneyValue(fieldPick(t,[/subtotal\s*[:#-]?\s*\$?([\d,]+(?:\.\d{2})?)/i,/net\s+amount\s*[:#-]?\s*\$?([\d,]+(?:\.\d{2})?)/i]))||lineItems.reduce((x,l)=>x+Number(l.extendedCost||0),0);
  const tax=moneyValue(fieldPick(t,[/(?:sales\s*)?tax(?:\s+amount)?\s*[:#-]?\s*\$?([\d,]+(?:\.\d{2})?)/i,/vat\s*[:#-]?\s*\$?([\d,]+(?:\.\d{2})?)/i]));
  const freight=moneyValue(fieldPick(t,[/(?:freight|shipping|delivery)\s*[:#-]?\s*\$?([\d,]+(?:\.\d{2})?)/i]));
  const discount=moneyValue(fieldPick(t,[/discount\s*[:#-]?\s*\$?([\d,]+(?:\.\d{2})?)/i]));
  const total=moneyValue(fieldPick(t,[/^(?:invoice\s*)?total(?:\s+amount)?\s*[:#-]?\s*\$?([\d,]+(?:\.\d{2})?)/im,/amount\s+due\s*[:#-]?\s*\$?([\d,]+(?:\.\d{2})?)/i,/balance\s+due\s*[:#-]?\s*\$?([\d,]+(?:\.\d{2})?)/i]))||Number((subtotal+tax+freight-discount).toFixed(2));
  return {vendor_name,vendor_number:fieldPick(t,[/vendor\s*(?:id|number|no\.?)\s*[:#-]?\s*(\S+)/i]),vendor_address:fieldPick(t,[/vendor\s+address\s*[:#-]\s*(.+)/i]),vendor_tax_id:fieldPick(t,[/(?:tax id|tin|vat id)\s*[:#-]?\s*(\S+)/i]),vendor_email:fieldPick(t,[/[\w.-]+@[\w.-]+/i]),vendor_phone:fieldPick(t,[/(?:phone|tel)\s*[:#-]?\s*([+()\d .-]{7,})/i]),invoice_number,invoice_date:normalizeInvoiceDate(invoiceDateRaw),due_date:normalizeInvoiceDate(dueDateRaw),payment_terms:fieldPick(t,[/terms\s*[:#-]?\s*([^\n]+)/i,/payment\s+terms\s*[:#-]?\s*([^\n]+)/i]),currency:fieldPick(t,[/currency\s*[:#-]?\s*(\w{3})/i])||'USD',po_number,po_numbers,contract_number:fieldPick(t,[/contract\s*(?:number|no\.?|#)?\s*[:#-]?\s*(\S+)/i]),receiving_number:fieldPick(t,[/receiv(?:ing|er)\s*(?:number|no\.?|#)?\s*[:#-]?\s*(\S+)/i]),packing_slip_number:fieldPick(t,[/packing\s*slip\s*(?:number|no\.?|#)?\s*[:#-]?\s*(\S+)/i]),subtotal,tax,freight,discount,total,line_items:lineItems};
}
function matchIncomingVendor(x){ const candidates=vendors.map(v=>{let score=0,reason=''; if(x.vendor_number&&x.vendor_number===v.id){score=100;reason='Vendor ID';} else if(x.vendor_tax_id&&x.vendor_tax_id===v.taxId){score=99;reason='Tax ID';} else if(x.vendor_name&&x.vendor_name.toLowerCase()===String(v.name).toLowerCase()){score=98;reason='Exact name';} else if(x.vendor_email&&x.vendor_email.toLowerCase()===String(v.email||'').toLowerCase()){score=96;reason='Email';} else {score=textScore(x.vendor_name,v.name); reason='Fuzzy name';} return {vendorId:v.id,vendorName:v.name,confidence:score,reason}; }).sort((a,b)=>b.confidence-a.confidence); return candidates[0]||{confidence:0}; }
function matchIncomingPo(x,vendorId){ const poNos=x.po_numbers?.length?x.po_numbers:[x.po_number].filter(Boolean); const candidates=purchaseOrders.filter(p=>(!poNos.length||poNos.includes(p.poNumber)||poNos.includes(p.id))&&(!vendorId||p.vendorId===vendorId)).map(p=>({...serializePo(p),confidence:(poNos.includes(p.poNumber)||poNos.includes(p.id)?98:80)})).sort((a,b)=>b.confidence-a.confidence); return candidates[0]||null; }
function incomingExceptions(row){ const e=[]; if(!row.vendorId) e.push({severity:'Error',message:'Vendor not found'}); if(!row.invoiceNumber) e.push({severity:'Error',message:'Invoice number not recognized'}); if(!row.invoiceDate) e.push({severity:'Error',message:'Invoice date not recognized'}); if(!(Number(row.amount||0)>0)) e.push({severity:'Error',message:'Invoice total not recognized'}); if(!(row.lines||[]).length) e.push({severity:'Error',message:'Line items not recognized'}); if(row.duplicateCandidates?.length) e.push({severity:'Error',message:'Duplicate invoice'}); if(!row.poNumber) e.push({severity:'Warning',message:'Missing PO'}); if(Number(row.matchConfidence||0)<90) e.push({severity:'Warning',message:'Confidence below review threshold'}); (row.lines||[]).forEach(l=>{ if(l.priceVariance) e.push({severity:'Error',message:`Price variance on ${l.description}`}); if(l.quantityVariance) e.push({severity:'Error',message:`Quantity variance on ${l.description}`}); }); return e; }
function processIncomingApBill(row){ const x=parseIncomingInvoiceText(row.ocrText||row.fileText||row.fileName||''); row.extracted=x; const vm=matchIncomingVendor(x); row.vendorId=vm.confidence>=90?vm.vendorId:''; row.vendorName=vm.vendorName||x.vendor_name; row.vendorMatch=vm; const po=matchIncomingPo(x,row.vendorId); row.poNumber=po?.poNumber||x.po_number||''; row.poNumbers=x.po_numbers||[]; row.poMatch=po?{poNumber:po.poNumber,confidence:po.confidence}:null; row.invoiceNumber=x.invoice_number; row.invoiceDate=x.invoice_date; row.dueDate=x.due_date||row.invoiceDate; row.subtotal=Number(x.subtotal||0); row.taxAmount=Number(x.tax||0); row.freightAmount=Number(x.freight||0); row.discount=Number(x.discount||0); row.amount=Number(x.total||0); row.currency=x.currency||'USD'; row.lines=(x.line_items||[]).map((l,i)=>{ const poLine=po?.lines?.find(pl=>pl.inventoryId===l.item_number||pl.vendorItemId===l.vendor_part_number||textScore(pl.description,l.description)>60); const confidence=poLine?Math.max(70,textScore(poLine.description,l.description)):Number(l.amount)>0?75:50; return {...l,lineNumber:i+1,poNumber:po?.poNumber||row.poNumber||'',poLineId:poLine?.id||'',inventoryId:poLine?.inventoryId||l.item_number||'',itemNumber:poLine?.inventoryId||l.item_number||'',expenseAccount:poLine?.expenseAccount||'5110',matchConfidence:confidence,priceVariance:poLine?Math.abs(Number(l.unitCost)-Number(poLine.unitCost))/Math.max(Number(poLine.unitCost),1)*100>Number(poSettings.priceTolerancePct||2):false,quantityVariance:poLine?Number(l.qty)>Number(poLine.qtyReceived||poLine.qtyOrdered||0)*(1+Number(poSettings.quantityTolerancePct||5)/100):false}; }); const parts=[vm.confidence,po?.confidence||row.poNumber?85:0,row.invoiceNumber?95:0,row.invoiceDate?95:0,row.amount>0?95:0,...row.lines.map(l=>l.matchConfidence||0)]; row.matchConfidence=Math.round(parts.reduce((a,b)=>a+b,0)/Math.max(parts.length,1)); row.duplicateCandidates=apDocuments.filter(d=>d.type==='Bill'&&d.vendorId===row.vendorId&&((row.invoiceNumber&&d.invoiceNumber===row.invoiceNumber)||(d.date===row.invoiceDate&&Number(d.amount)===row.amount))).map(d=>({id:d.id,confidence:d.invoiceNumber===row.invoiceNumber?97:90})); row.exceptions=incomingExceptions(row); row.status=row.exceptions.some(e=>e.severity==='Error')?'Exception':(row.matchConfidence>=90?'Matched':'Review Required'); row.auditTrail.push({date:new Date().toISOString(),event:'AI extraction and matching completed',status:row.status}); return row; }
function createBillFromIncoming(row){ if(row.billId) return apDocuments.find(d=>d.id===row.billId); if((row.exceptions||[]).some(e=>e.severity==='Error')) throw new Error('Resolve incoming bill exceptions before creating AP Bill.'); const vendor=vendors.find(v=>v.id===row.vendorId); if(!vendor) throw new Error('Vendor is required'); const billLines=(row.lines||[]).map(l=>({...l,qty:l.qty,unitCost:l.unitCost,extendedCost:l.extendedCost,amount:l.extendedCost,poNumber:l.poNumber||row.poNumber,receiptNumber:l.receiptNumber||''})); if(Number(row.taxAmount||0)>0) billLines.push({description:'Recognized invoice tax',qty:1,unitCost:Number(row.taxAmount||0),extendedCost:Number(row.taxAmount||0),amount:Number(row.taxAmount||0),expenseAccount:'5110',lineType:'Tax'}); if(Number(row.freightAmount||0)>0) billLines.push({description:'Recognized invoice freight',qty:1,unitCost:Number(row.freightAmount||0),extendedCost:Number(row.freightAmount||0),amount:Number(row.freightAmount||0),expenseAccount:'5110',lineType:'Freight'}); if(Number(row.discount||0)>0) billLines.push({description:'Recognized invoice discount',qty:1,unitCost:-Number(row.discount||0),extendedCost:-Number(row.discount||0),amount:-Number(row.discount||0),expenseAccount:'5110',lineType:'Discount'}); const doc={type:'Bill',vendorId:vendor.id,vendorName:vendor.name,date:row.invoiceDate,postDate:row.invoiceDate,dueDate:row.dueDate,terms:row.extracted?.payment_terms||vendor.terms,invoiceNumber:row.invoiceNumber,vendorRef:row.invoiceNumber,amount:row.amount,balance:row.amount,hold:true,currency:row.currency,invoicePdfAttached:true,attachmentName:row.fileName,attachments:[{name:row.fileName,type:row.fileType||'application/pdf',source:'Incoming AP Bills'}],createdBy:row.createdBy||'ap.clerk',ocrCreated:true,lines:billLines,source:'Incoming AP Bills',aiConfidence:row.matchConfidence}; validateApBillAmount(doc); const id=`BILL-${String(apDocuments.length+1001).padStart(4,'0')}`; doc.id=id; doc.status='Draft'; doc.postPeriod=periodFromDate(doc.postDate); doc.approvalRequired=apApprovalRequired(doc); apDocuments.push(doc); row.billId=id; row.status='Completed'; row.auditTrail.push({date:new Date().toISOString(),event:`AP Bill ${id} created on hold`,status:row.status}); addWorkflowAudit({billId:id,action:'Create from Incoming AP Bill',userId:'admin',fromStatus:'Incoming',toStatus:'Draft',comments:`Source file ${row.fileName}`}); return doc; }
...
const server=http.createServer(async(req,res)=>{const {pathname,query}=parse(req.url,true); const method=req.method||'GET'; try{
 ...
 if(method==='GET'&&pathname==='/api/ap/incoming-bills') return json(res,200,incomingApBills);
 if(method==='GET'&&pathname.startsWith('/api/ap/incoming-bills/')){ const id=pathname.split('/').pop(); const row=incomingApBills.find(x=>x.id===id); return row?json(res,200,row):json(res,404,{error:'Incoming bill not found'}); }
 if(method==='POST'&&pathname==='/api/ap/incoming-bills'){ const b=await body(req); const files=Array.isArray(b.files)?b.files:[b]; const created=files.map(f=>{ const row={id:`INAP-${String(incomingApBillSeq++).padStart(6,'0')}`,status:'New',fileName:f.fileName||f.name||'invoice.pdf',fileType:f.fileType||f.type||'application/pdf',createdBy:f.createdBy||'ap.clerk',createdDate:new Date().toISOString(),ocrText:f.ocrText||f.text||f.fileText||'',auditTrail:[{date:new Date().toISOString(),event:'File uploaded',status:'New'}]}; incomingApBills.unshift(row); processIncomingApBill(row); return row; }); return json(res,201,created.length===1?created[0]:created); }
 if(method==='POST'&&pathname.startsWith('/api/ap/incoming-bills/')&&pathname.endsWith('/reprocess')){ const id=pathname.split('/').at(-2); const row=incomingApBills.find(x=>x.id===id); if(!row) return json(res,404,{error:'Incoming bill not found'}); row.status='Processing'; row.auditTrail.push({date:new Date().toISOString(),event:'Manual reprocess requested',status:row.status}); return json(res,200,processIncomingApBill(row)); }
 if(method==='PUT'&&pathname.startsWith('/api/ap/incoming-bills/')){ const id=pathname.split('/').pop(); const row=incomingApBills.find(x=>x.id===id); if(!row) return json(res,404,{error:'Incoming bill not found'}); Object.assign(row,await body(req)); row.exceptions=incomingExceptions(row); row.status=row.exceptions.some(e=>e.severity==='Error')?'Exception':'Review Required'; row.auditTrail.push({date:new Date().toISOString(),event:'User corrections saved',status:row.status}); return json(res,200,row); }
 if(method==='POST'&&pathname.startsWith('/api/ap/incoming-bills/')&&pathname.endsWith('/create-bill')){ const id=pathname.split('/').at(-2); const row=incomingApBills.find(x=>x.id===id); if(!row) return json(res,404,{error:'Incoming bill not found'}); try{return json(res,201,{incomingBill:row,bill:createBillFromIncoming(row)});}catch(e){return json(res,400,{error:e.message});} }
