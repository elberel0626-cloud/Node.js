...
function createReceiptFromPo(poId, opts={}){
  ...
  receipt.jeReference=createPostedJournal({module:'Inventory',description:`Purchase Receipt ${receipt.id}`,postPeriod:pp,transactionDate:postDate,sourceRef:receipt.id,lines:jeLines}); receipt.batchNbr=receipt.jeReference;
...
function serializePurchaseReceipt(r){
  ...
  const financialDetails=journalEntries.filter(j=>j.sourceRef===r.id||j.jeNumber===r.jeReference).flatMap(j=>(j.lines||[]).map(l=>({jeNumber:j.jeNumber,batchNumber:j.batchNumber||j.jeNumber,account:l.account,description:l.description||j.description,debit:l.debit,credit:l.credit,sourceReference:l.sourceReference||r.id,postDate:j.transactionDate,postPeriod:j.financialPeriod,status:j.status})));
  const invTx=inventoryTransactions.filter(t=>t.referenceNumber===r.id||t.sourceReference===r.id||t.id===r.id);
  return {...r,documentType:'Purchase Receipt',receiptId:r.receiptId||r.id,receiptID:r.receiptID||r.id,receiptNbr:r.receiptNbr||r.receiptNumber||r.id,vendorID:r.vendorId,warehouseID:r.warehouseId||r.warehouse,financialPeriod:r.financialPeriod||r.postPeriod,batchNbr:r.batchNbr||r.jeReference,total:lines.reduce((t,l)=>t+Number(l.extendedCost||0),0),qtyReceived:lines.reduce((t,l)=>t+Number(l.receiptQty||0),0),qtyBilled:lines.reduce((t,l)=>t+Number(l.qtyBilled||0),0),lines,linkedBills,financialDetails,inventoryTransactions:invTx,purchaseOrders:[{poId:r.poId,poNumber:r.poNumber,vendorId:r.vendorId,vendorName:r.vendorName,status:r.status}]};
}
function serializeShipment(sh){
  ...
  const financialDetails=journalEntries.filter(j=>j.sourceRef===sh.id||j.sourceRef===sh.shipmentNumber||(j.lines||[]).some(l=>l.sourceReference===sh.id||l.sourceReference===sh.shipmentNumber)).flatMap(j=>(j.lines||[]).filter(l=>!l.sourceReference||l.sourceReference===sh.id||l.sourceReference===sh.shipmentNumber).map(l=>({jeNumber:j.jeNumber,batchNumber:j.batchNumber,account:l.account,description:l.description||j.description,debit:l.debit,credit:l.credit})));
