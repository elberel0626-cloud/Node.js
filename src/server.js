...
function documentStatus(exceptions, duplicate){ return duplicate?'Duplicate':exceptions.length?'Needs Review':'Ready'; }
...
const invoiceRecognitionService = new InvoiceRecognitionService({documents:apIncomingVendorBills,buildIncomingRecord,updateDocument:updateIncomingDocument,vendors,purchaseOrders,purchaseOrderLines});
async function receiveIncomingDocument(upload){
  const id=nextIncomingId();
  const fileName=upload.fileName||upload.attachmentName||upload.name||'invoice.pdf';
  const rec={id,source:upload.source||'PDF Upload',senderEmail:upload.senderEmail||'',receivedDate:upload.receivedDate||new Date().toISOString(),assignedUser:upload.assignedUser||'ap.clerk',aiConfidence:0,documentType:'',fileName,uploadedBy:upload.uploadedBy||'ap.clerk',uploadedAt:new Date().toISOString(),status:'Received',attachment:{name:fileName,mimeType:upload.mimeType||'application/pdf',dataUrl:upload.fileData||upload.attachmentDataUrl||'',text:upload.text||''},extracted:{},confidence:{},vendorMatch:{},poMatch:{status:'PO Not Found',summary:'Not processed',lines:[]},exceptions:[],draftBill:{},billId:'',auditTrail:[{date:new Date().toISOString(),user:upload.uploadedBy||'ap.clerk',action:'Received',details:`Received ${fileName}`}],processingDetails:{embeddedTextDetected:false,ocrUsed:false,pagesProcessed:0,vendorMatchConfidence:0,poMatchResult:'Not processed',lineTableDetected:false,errorDetails:''}};
  apIncomingVendorBills.unshift(rec);
  return invoiceRecognitionService.processIncomingDocument(id);
}
...
const server=http.createServer(async(req,res)=>{const {pathname,query}=parse(req.url,true); const method=req.method||'GET'; try{
 ...
 if(method==='GET'&&pathname==='/api/ap/incoming-vendor-bills') return json(res,200,apIncomingVendorBills);
 if(method==='GET'&&pathname==='/api/ap/incoming-vendor-bills/dashboard') return json(res,200,incomingDashboard());
 if(method==='GET'&&pathname==='/api/ap/incoming-vendor-bills/settings') return json(res,200,apAutomationSettings);
 if(method==='PUT'&&pathname==='/api/ap/incoming-vendor-bills/settings'){ Object.assign(apAutomationSettings,await body(req)); return json(res,200,apAutomationSettings); }
 if(method==='POST'&&pathname==='/api/ap/incoming-vendor-bills'){ const b=await body(req); return json(res,201,await receiveIncomingDocument(b)); }
 if(method==='POST'&&pathname==='/api/ap/incoming-vendor-bills/email'){ const b=await body(req); return json(res,201,await receiveIncomingDocument({...b,source:'Email Inbox',fileName:b.attachmentName||'email-invoice.pdf'})); }
 if(method==='GET'&&pathname==='/api/ap/incoming-vendor-bills/tables') return json(res,200,{IncomingDocuments,DocumentFields,DocumentFieldCoordinates,VendorLearningProfiles,RecognitionCorrections});
 if(method==='GET'&&pathname.startsWith('/api/ap/incoming-vendor-bills/')){ const id=pathname.split('/').pop(); const r=apIncomingVendorBills.find(x=>x.id===id); if(!r) return json(res,404,{error:'Incoming invoice not found'}); return json(res,200,r); }
 if(method==='PUT'&&pathname.startsWith('/api/ap/incoming-vendor-bills/')){ const id=pathname.split('/').pop(); const r=apIncomingVendorBills.find(x=>x.id===id); if(!r) return json(res,404,{error:'Incoming invoice not found'}); const b=await body(req); const before={...r.extracted}; Object.assign(r.extracted,b.extracted||{}); Object.assign(r.draftBill,b.draftBill||{}); r.status=b.status||r.status; Object.entries(b.extracted||{}).forEach(([field,correct])=>{if(String(before[field]??'')!==String(correct??'')) RecognitionCorrections.push({documentId:r.id,vendorId:r.vendorMatch?.vendorId,field,oldValue:before[field],correctValue:correct,date:new Date().toISOString(),user:b.user||'ap.clerk'});}); r.auditTrail.push({date:new Date().toISOString(),user:b.user||'ap.clerk',action:'Correction',details:'User corrected extracted values for AI learning.'}); return json(res,200,r); }
 if(method==='POST'&&pathname.endsWith('/create-bill')&&pathname.startsWith('/api/ap/incoming-vendor-bills/')){ const id=pathname.split('/').at(-2); const r=apIncomingVendorBills.find(x=>x.id===id); if(!r) return json(res,404,{error:'Incoming invoice not found'}); if(r.billId) return json(res,200,{billId:r.billId,bill:serializeApDoc(apDocuments.find(d=>d.id===r.billId))}); if(r.exceptions.some(e=>e.type==='Duplicate Invoice')&&!((await body(req)).overrideDuplicate)) return json(res,400,{error:'Potential Duplicate Invoice Detected'}); const d={...r.draftBill,id:`BILL-${String(apDocuments.length+1001).padStart(4,'0')}`,status:'Draft',source:'Incoming Vendor Bills',incomingInvoiceId:r.id,approvals:[],history:[]}; apDocuments.push(d); r.billId=d.id; r.status='Converted'; r.auditTrail.push({date:new Date().toISOString(),user:'ap.clerk',action:'Bill Creation',details:`AP Bill draft ${d.id} created.`}); return json(res,201,{billId:d.id,bill:serializeApDoc(d)}); }
