...
if(method==='POST'&&pathname==='/api/ar/documents/post'){
 ...
 if(method==='POST'&&pathname==='/api/ar/release/post-selected'){ const b=await body(req); const ids=b.ids||[]; const docs=ids.map(id=>arDocuments.find(x=>x.id===id)).filter(Boolean); docs.forEach(normalizeArStatus); const toPost=docs.filter(d=>d.status==='Saved'); const zeroPayment=toPost.find(d=>d.type==='Payment'&&Number(d.amount)<=0); if(zeroPayment)return json(res,400,{error:'Payment amount must be greater than $0.00.'}); toPost.forEach(d=>{ if(['Invoice','Credit Memo','Debit Memo'].includes(d.type)) applyTaxToArDocument(d, customers.find(c=>c.id===d.customerId)); validateTaxBeforePosting(d); validateSourceAndGlOpen('AR',d.postPeriod||periodFromDate(d.postDate||d.date)); }); const updated=[]; for(const d of toPost){ d.posted=true; postJE(d,false); normalizeArStatus(d); updated.push(d);} return json(res,200,{posted:updated.length,documents:updated}); }
