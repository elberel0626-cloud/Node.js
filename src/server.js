...
const server=http.createServer(async(req,res)=>{const {pathname,query}=parse(req.url,true); const method=req.method||'GET'; try{
 ...
 if(method==='POST'&&pathname==='/api/sales-orders/release-documents'){ const b=await body(req); const ids=b.ids||[]; const posted=[]; for(const id of ids){ const inv=arDocuments.find(d=>d.id===id&&d.type==='Invoice'); if(inv&&inv.status==='Saved'){ applyTaxToArDocument(inv, customers.find(c=>c.id===inv.customerId)); validateTaxBeforePosting(inv); validateSourceAndGlOpen('AR',inv.postPeriod||periodFromDate(inv.postDate||inv.date)); inv.posted=true; postJE(inv,false); normalizeArStatus(inv); const rel=salesOrderInvoices.find(x=>x.invoiceId===inv.id); if(rel){ rel.status=inv.status; rel.openBalance=inv.balance; } posted.push(inv.id); } } return json(res,200,{posted}); }
 ...
 if(method==='POST'&&pathname==='/api/ar/payments/apply'){
  ...
  for(const app of applications){
    if(app.appliedToType==='Sales Order'){
      ...
      normalizeArStatus(inv); if(!inv.posted||!['Open','Closed'].includes(inv.status)) return json(res,400,{error:'Payment cannot be applied because this invoice has not been posted.'}); if(inv.status!=='Open'||toNumber(inv.balance)<=0) return json(res,400,{error:'Only open invoices can be applied'}); if(app.amount>toNumber(inv.balance)) return json(res,400,{error:'Applied payment cannot exceed document balance'});
    }
  }
...
if(method==='POST'&&pathname==='/api/ar/documents/post'){
 const {id}=await body(req); const d=arDocuments.find(x=>x.id===id); if(!d)return json(res,404,{error:'Not found'}); const pp=d.postPeriod||periodFromDate(d.postDate||d.date); validateSourceAndGlOpen('AR',pp); if(['Invoice','Credit Memo','Debit Memo'].includes(d.type)) applyTaxToArDocument(d, customers.find(c=>c.id===d.customerId)); normalizeArStatus(d); if(d.status==='Voided') return json(res,400,{error:'Voided doc'}); if(d.status!=='Saved') return json(res,400,{error:'Only Saved transactions can be posted'}); if(d.type==='Payment'){for(const application of (d.applications||[])){if(application.invoiceId){const invoice=arDocuments.find(x=>x.id===application.invoiceId); normalizeArStatus(invoice); if(!invoice?.posted||!['Open','Closed'].includes(invoice.status)) return json(res,400,{error:'Payment cannot be applied because this invoice has not been posted.'});}}} try{ validateTaxBeforePosting(d); }catch(e){ return json(res,400,{error:e.message}); } d.posted=true;
