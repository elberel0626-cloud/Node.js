...
async function receiveIncomingDocument(upload){
  ...
  setTimeout(()=>invoiceRecognitionService.processIncomingDocument(id),0);
  return rec;
}
...
const server=http.createServer(async(req,res)=>{const {pathname,query}=parse(req.url,true); const method=req.method||'GET'; try{
 ...
 if(method==='POST'&&pathname.startsWith('/api/ap/incoming-documents/')&&pathname.endsWith('/retry-recognition')){ const id=pathname.split('/').at(-2); const r=apIncomingVendorBills.find(x=>x.id===id); if(!r) return json(res,404,{error:'Incoming document not found'}); r.processingDetails.retryCount=Number(r.processingDetails.retryCount||0)+1; setTimeout(()=>invoiceRecognitionService.processIncomingDocument(id),0); return json(res,202,{id,status:'QUEUED'}); }
