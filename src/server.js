...
function parseIncomingInvoiceText(text=''){
  const t=String(text||''); const pick=(re)=>{const m=t.match(re); return m?String(m[1]).trim():'';};
  const lineItems=[]; const lineRe=/^\s*(\d+(?:\.\d+)?)\s*[|,\t ]+(.+?)\s*[|,\t ]+(\d+(?:\.\d{1,4})?)\s*(?:[|,\t ]+(\d+(?:\.\d{1,2})?))?\s*$/gmi; let m;
  while((m=lineRe.exec(t))) { const qty=Number(m[1]), unit=Number(m[3]), amount=Number(m[4]||qty*unit); if(qty>0&&unit>=0&&!/qty|item|price|amount/i.test(m[2])) lineItems.push({qty,description:m[2].trim(),unitCost:unit,extendedCost:amount,amount,uom:'EA'}); }
  const subtotal=Number(pick(/subtotal\s*[:#-]?\s*\$?([\d,.]+)/i).replace(/,/g,''))||lineItems.reduce((x,l)=>x+Number(l.extendedCost||0),0);
  const tax=Number(pick(/tax(?: amount)?\s*[:#-]?\s*\$?([\d,.]+)/i).replace(/,/g,''))||0;
  const freight=Number(pick(/freight|shipping\s*[:#-]?\s*\$?([\d,.]+)/i).replace(/,/g,''))||0;
  const total=Number(pick(/total(?: amount)?\s*[:#-]?\s*\$?([\d,.]+)/i).replace(/,/g,''))||subtotal+tax+freight;
  return {vendor_name:pick(/vendor(?: name)?\s*[:#-]?\s*(.+)/i)||pick(/^(.+?)\s+invoice/im),vendor_number:pick(/vendor (?:id|number)\s*[:#-]?\s*(\S+)/i),vendor_tax_id:pick(/tax id\s*[:#-]?\s*(\S+)/i),vendor_email:pick(/[\w.-]+@[\w.-]+/i),invoice_number:pick(/invoice (?:number|#|no\.)\s*[:#-]?\s*(\S+)/i)||pick(/inv(?:oice)?[-# ]+(\S+)/i),invoice_date:pick(/invoice date\s*[:#-]?\s*(\d{4}-\d{2}-\d{2}|\d{1,2}\/\d{1,2}\/\d{2,4})/i),due_date:pick(/due date\s*[:#-]?\s*(\d{4}-\d{2}-\d{2}|\d{1,2}\/\d{1,2}\/\d{2,4})/i),payment_terms:pick(/terms\s*[:#-]?\s*([\w\s]+?)(?:\n|$)/i),currency:pick(/currency\s*[:#-]?\s*(\w{3})/i)||'USD',po_number:pick(/(?:po|purchase order)(?: number|#)?\s*[:#-]?\s*(PO-?\d+)/i),subtotal,tax,total,line_items:lineItems};
}
