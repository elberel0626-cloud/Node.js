...
export function applyManufacturingAgent3PlanningPatch(source){
  ...
  source=replaceOnceOrAlready(
    source,
    "const planNeed=(itemId,gross,dueDate,source,stack=[])=>{if(stack.includes(itemId))throw new Error(`MRP encountered a circular BOM: ${[...stack,itemId].join(' -> ')}`);const current=Number(supplyRemaining.get(itemId)||0),used=Math.min(current,gross);supplyRemaining.set(itemId,qty(current-used));let short=Math.max(0,gross-used);if(short<=0)return;const itemRow=item(itemId);short=roundPlanQuantity(itemRow,short);const bom=effectiveBom(itemId,nowDate());if(bom){addPlan('Make',itemId,short,dueDate,source);for(const component of materialRequirements(bom,short)){planNeed(component.itemId,component.requiredQty,dateMinusDays(dueDate,itemRow?.leadTimeDays||0),{sourceType:'BOM Demand',sourceReference:itemId,parentSource:source},[...stack,itemId]);}}else addPlan('Buy',itemId,short,dueDate,source);};",
