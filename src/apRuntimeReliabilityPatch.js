...
export function applyApRuntimeReliabilityPatch(source){
  ...
  source=replaceOnce(source,
    "function serializeApDoc(doc){ const lines=",
    "function serializeApDoc(doc){ doc=doc&&typeof doc==='object'?{...doc}:{}; doc.lines=Array.isArray(doc.lines)?doc.lines.map(line=>line&&typeof line==='object'?line:{}):[]; doc.approvals=Array.isArray(doc.approvals)?doc.approvals:[]; doc.history=Array.isArray(doc.history)?doc.history:[]; if(doc.poQuantityException&&typeof doc.poQuantityException==='object')doc.poQuantityException={...doc.poQuantityException,lines:Array.isArray(doc.poQuantityException.lines)?doc.poQuantityException.lines:[]}; const serializedPoMatch=(()=>{try{return evaluatePoThreeWayMatch(doc);}catch(error){console.error('AP bill PO match serialization failed',doc.id||'',error.message);return{hasPo:doc.lines.some(line=>line.poNumber||line.sourcePoId||line.poId),status:'Match Exception',postable:false,lines:[],totals:{invoiceQty:0,matchedQty:0,shortQty:0,priceVariance:0,quantityVariance:0},serializationError:true};}})(); const lines=",
    'legacy AP document serialization safety');
  source=replaceOnce(source,
    "threeWayMatched:isThreeWayMatchedWithinTolerance(doc),threeWayMatch:evaluatePoThreeWayMatch(doc),matchStatus:evaluatePoThreeWayMatch(doc).status,poQuantityException:",
    "threeWayMatched:serializedPoMatch.hasPo&&serializedPoMatch.postable,threeWayMatch:serializedPoMatch,matchStatus:serializedPoMatch.status,poQuantityException:",
    'serialized AP document uses safe PO match result');
  source=replaceOnce(source,
